...
describe('views/account_recovery_confirm_key', () => {
  ...
  const accountResetToken = '237af7c984607e23fadfc0f75d214ac50555d2e5c50acecae6c184d0ebe202c9';
  const token = 'feed';
  ...
  describe('submit', () => {
    ...
    it('should call passwordForgotVerifyCode', () => {
      assert.equal(account.passwordForgotVerifyCode.callCount, 1, 'called create forgot verify code');
    ...
    it('should call getRecoveryBundle', () => {
      assert.equal(account.getRecoveryBundle.callCount, 1, 'called get recovery bundle');
    ...
    it('should navigate to `/account_recovery_reset_password` with successful key', () => {
      ...
      assert.equal(args[0], '/account_recovery_reset_password', 'navigated to complete password');
      assert.equal(args[1].accountResetToken, accountResetToken, 'accountResetToken set');
  ...
  describe('should set token expired and re-render', () => {
    ...
    it('should mark token expired and re-render', () => {
      assert.equal(view._verificationInfo.markExpired.callCount, 1, 'verification marked expired');
  ...
  describe('should navigate to regular password reset with lost recovery key', () => {
    ...
    it('should navigate to password reset ', () => {
      const args = view.navigate.args[0];
      assert.equal(args[0], '/complete_reset_password', 'navigated to complete password');
