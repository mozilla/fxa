  ...
  describe('enabled', () => {
    ...
    describe('accountPref', () => {
      ...
      it('submits a ping with the account_pref_two_step_auth_phone_verify_view event name', async () => {
        ...
        const spy = sandbox.spy(
          accountPref.twoStepAuthPhoneVerifyView,
          'record'
        );
        await GleanMetrics.isDone();
        sinon.assert.calledOnce(setEventNameStub);
    ...
    describe('pair2', () => {
      it('submits a ping with the dtm_desktop_pair_success_view event name', async () => {
        GleanMetrics.dtmDesktop.pairSuccessView();
        const spy = sandbox.spy(dtmDesktop.pairSuccessView, 'record');
        await GleanMetrics.isDone();
        sinon.assert.calledOnce(setEventNameStub);
        sinon.assert.calledWith(
          setEventNameStub,
          'dtm_desktop_pair_success_view'
        );
        sinon.assert.calledOnce(spy);
      });

      it('submits a ping with the dtm_mobile_pair_success_view event name', async () => {
        GleanMetrics.dtmMobile.pairSuccessView();
        const spy = sandbox.spy(dtmMobile.pairSuccessView, 'record');
        await GleanMetrics.isDone();
        sinon.assert.calledOnce(setEventNameStub);
        sinon.assert.calledWith(
          setEventNameStub,
          'dtm_mobile_pair_success_view'
        );
        sinon.assert.calledOnce(spy);
      });
