...
describe('MeteringSweepRepository', () => {
  ...
  describe('findWindowCandidates', () => {
    const params = {
      ...
      ingestedUntil: new Date('2026-05-15T12:00:00.000Z'),
      ...
      expect(clickHouseClient.query).toHaveBeenCalledWith(
        expect.objectContaining({
          params: {
            ...
            ingestedUntil: '2026-05-15 12:00:00.000',
    ...
    it('restricts the scan to subjects with recent activity', async () => {
      ...
      expect(sql).toContain(
        "ingested_at < {ingestedUntil:DateTime64(3, 'UTC')}"
      );
  ...
  describe('findWatermark', () => {
    it('binds the meter key', async () => {
      clickHouseClient.query.mockResolvedValue([]);

      await repository.findWatermark(scope);

      expect(clickHouseClient.query).toHaveBeenCalledWith(
        expect.objectContaining({
          params: { clientId: 'vpn', slug: 'tokens' },
        })
      );
    });

    it('breaks updated_at ties on the watermark itself', async () => {
      clickHouseClient.query.mockResolvedValue([]);

      await repository.findWatermark(scope);

      const { sql } = clickHouseClient.query.mock.calls[0][0];
      expect(sql).toContain('argMax(watermark, (updated_at, watermark))');
    });

    it('returns null when no watermark has been recorded', async () => {
      clickHouseClient.query.mockResolvedValue([]);

      await expect(repository.findWatermark(scope)).resolves.toBeNull();
    });

    it('returns the recorded watermark', async () => {
      clickHouseClient.query.mockResolvedValue([
        { watermark: '2026-05-15T11:55:00.000Z' },
      ]);

      await expect(repository.findWatermark(scope)).resolves.toBe(
  ...
  describe('findActiveMeters', () => {
    const params = { ingestedSince: new Date('2026-05-14T12:00:00.000Z') };

    it('binds the ingest lookback', async () => {
      clickHouseClient.query.mockResolvedValue([]);

      await repository.findActiveMeters(params);

      expect(clickHouseClient.query).toHaveBeenCalledWith(
        expect.objectContaining({
          params: { ingestedSince: '2026-05-14 12:00:00.000' },
        })
      );
    });

    it('reads recent events and every recorded watermark', async () => {
      clickHouseClient.query.mockResolvedValue([]);

      await repository.findActiveMeters(params);

      const { sql } = clickHouseClient.query.mock.calls[0][0];
      expect(sql).toContain(`FROM ${EVENTS_TABLE}`);
      expect(sql).toContain(`FROM ${WATERMARKS_TABLE}`);
      expect(sql).toContain(
        "ingested_at >= {ingestedSince:DateTime64(3, 'UTC')}"
      );
    });

    it('maps rows to meter keys', async () => {
      clickHouseClient.query.mockResolvedValue([
        { client_id: 'relay', slug: 'bandwidth' },
        { client_id: 'vpn', slug: 'tokens' },
      ]);

      await expect(repository.findActiveMeters(params)).resolves.toEqual([
        { clientId: 'relay', slug: 'bandwidth' },
        { clientId: 'vpn', slug: 'tokens' },
      ]);
    });
    ...
    it('propagates a ClickHouse failure', async () => {
      ...
      await expect(repository.findActiveMeters(params)).rejects.toThrow(
        ClickHouseError
      );
    });
  });
