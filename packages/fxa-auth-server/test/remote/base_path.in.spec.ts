...
describe.each(testVersions)(
  ...
  ({ version, tag }) => {
    ...
    // The version routes call `git rev-parse HEAD` with cwd set to the
    // repo's .git path.  In git worktrees .git is a file, not a directory,
    // which causes the git command to fail and the route to return 500.
    // We skip these tests when running inside a worktree.
    const isWorktree = !fs.statSync(
      path.resolve(__dirname, '..', '..', '..', '..', '.git')
    ).isDirectory();

    const itOrSkip = isWorktree ? it.skip : it;

    itOrSkip('"/" returns valid version information', async () => {
    ...
    itOrSkip('"/__version__" returns valid version information', async () => {
