...
export async function attachAuthorityDiagnostics(
  ...
  try {
    await client.setContext('content');
    await save('authority-url.txt', await client.getUrl(), 'text/plain');

    // The containers log their channel traffic with a `[pair2]` prefix, which
    // is what says whether pair:supp:request ever arrived. `wrappedJSObject`
    // gets past Marionette's Xray vision, which hides page-defined properties.
