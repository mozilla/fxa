 ...
 */
interface CacheShim {
  writeQuery(options: { query: unknown; data: unknown }): void;
  modify(options: Record<string, unknown>): void;
  identify(object: Record<string, unknown>): string;
}

export const cache: CacheShim = {
  writeQuery(_options: { query: unknown; data: unknown }) {},
  modify(_options: Record<string, unknown>) {},
  identify(_object: Record<string, unknown>) {
    return '';
  },
};
