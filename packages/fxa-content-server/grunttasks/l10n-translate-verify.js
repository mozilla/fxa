...
module.exports = function (grunt) {
  grunt.registerTask('l10n-translate-verify', 'Run the Valid URL parser on the generated l10n files', [
    'clean',
    'selectconfig:dist',
    'l10n-generate-pages',
    'l10n-json-to-html',
  ...
  grunt.registerMultiTask('verify-urls', 'Verify whether the translated urls are valid', function (){
    ...
    var hasErrors = 0;
    var urlRegex = /(href="[\w/:\\\.%()]+")/ig;
    var idRegex = /(?:<a).*id="[\w\-]+"/ig;
      ...
      if (urlList !== null) {
        urlList.forEach(function (url) {
          if (urlArray.indexOf(url) === -1) {
            grunt.log.error('Found an url which should not be there', url, src);
    ...
    if (hasErrors !== 0) {
      ...
      grunt.log.writeln('Checked ' + fileArray.length + ' files for valid hrefs and ids and found nothing wrong.');
    }
  });
  ...
  grunt.registerMultiTask('generate-valid-urls', 'Generate the list of valid urls and ids for translation', function () {
    var urlRegex = /(href=\\?(?:"|')[\w/:\\\.%()]+(?:"|'))/ig;
    var idRegex = /(?:<a).*id=\\"[\w\-]+\\"/ig;
    var urlArray = [];
    var idArray = [];
    ...
    grunt.log.writeln('Generated urls and ids from ' + this.files[0].src.length + ' files');
  });
