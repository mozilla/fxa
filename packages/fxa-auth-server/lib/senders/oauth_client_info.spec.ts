...
describe('lib/senders/oauth_client_info:', () => {
  describe('fetch:', () => {
    ...
    it('fetches and memory caches client information', async () => {
      ...
      expect(mockLog.trace.mock.calls[0][0]).toBe('fetch.start');
      expect(mockLog.trace.mock.calls[1][0]).toBe('fetch.usedServer');
      expect(mockLog.trace.mock.calls[2]).toBeUndefined();

      // second call is cached
      const res2 = await fetch('24bdbfa45cd300c5');
      expect(mockLog.trace.mock.calls[2][0]).toBe('fetch.start');
      expect(mockLog.trace.mock.calls[3][0]).toBe('fetch.usedCache');
