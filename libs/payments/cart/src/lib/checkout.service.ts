...
export class CheckoutService {
  constructor(
    ...
    private gleanManager: PaymentsGleanManager,
  ...
  async postPaySteps(args: {
    ...
    if (args.isCancelInterstitialOffer && args.requestArgs) {
      try {
        const metricsOptOut = await retrieveOptOut(this.accountManager, uid);
        if (!metricsOptOut) {
          const additionalData = await retrieveAdditionalMetricsData(
            this.productConfigurationManager,
            this.cartManager,
            args.requestArgs.params
          );
          const { nimbusUserId } = await getNimbusUserId(this.nimbusManager, {
            ...
            experimentationId: args.requestArgs.experimentationId,
            experimentationPreview:
              args.requestArgs?.searchParams?.['experimentationPreview'] ===
              'true',
          });

          this.gleanManager.recordGenericEvent(
            'recordCancelInterstitialOfferRedeemed',
            {
              commonMetricsData: args.requestArgs,
              ...additionalData,
    ...
    if (intent) {
      if (intent.status === 'requires_action') {
        ...
        const isCancelInterstitialOffer =
          ...
          attribution.session_entrypoint === 'subscription-management';
