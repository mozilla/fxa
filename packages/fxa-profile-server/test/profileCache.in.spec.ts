...
describe('#integration - profile cache', () => {
  ...
  it('should cache profile info initially, and invalidate cache after 2 seconds', async () => {
    const userid = uid();
  ...
  it('should invalidate cache when display name is updated', async () => {
    const userid = uid();
    await Server.server.initialize();
    // first req, store last modified header
    mock.email(MOZILLA_EMAIL);
  ...
  it('should invalidate cache when auth-server profileChangedAt is greater than cached version', async () => {
    const userid = uid();
    await Server.server.initialize();
    // first req, store last modified header
  ...
  it('should not cache reads with unusual sets of scopes', async () => {
    const userid = uid();
  ...
  it('should separately cache full and partial profile reads', async () => {
    const userid = uid();
    ...
    const avatarUrl = uploadBody.url;
    ...
    expect(res2.headers['last-modified']).toBeTruthy();
    const lastModifiedFull = res2.headers['last-modified'];
  ...
  it('should not leak unauthorized data from cached profile', async () => {
    const userid = uid();
    await Server.server.initialize();
    mock.coreProfile({
      email: MOZILLA_EMAIL,
