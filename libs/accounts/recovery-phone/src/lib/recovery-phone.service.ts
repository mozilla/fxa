...
export class RecoveryPhoneService {
  ...
  public async setupPhoneNumber(
    ...
    console.log(
      '!!! setup phone number 2',
      this.config.sms?.validNumberPrefixes,
      phoneNumber
    );
    ...
    if (this.config.sms?.validNumberPrefixes) {
      ...
      if (!allowed) {
        this.metrics.increment('number_prefix.blocked');
        throw new RecoveryNumberNotSupportedError(phoneNumber);
      } else {
        this.metrics.increment('number_prefix.allowed');
      }
    }
    ...
    if (this.config.sms?.validCountryCodes) {
      const lookupData = await (async () => {
        try {
          ...
          this.metrics.increment('country_code.lookup_failed');
      ...
      if (!allowed) {
        ...
        this.metrics.increment('country_code.blocked');
        throw new RecoveryNumberNotSupportedError(phoneNumber);
      } else {
        this.metrics.increment('country_code.allowed');
      }
    }
