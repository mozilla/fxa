...
const SubRow = ({
  ...
  const LearnAboutSimSwapRiskLink = () => {
      ...
      >
        <div className="flex flex-row justify-between flex-1 flex-wrap items-center">
          ...
          <div>
            ...
            {!localizedDescription &&
              !localizedInfoMessage &&
              linkExternalProps && <LearnAboutSimSwapRiskLink />}
          </div>
          {localizedDescription && (
            <p className="text-xs w-full mx-2 mt-1">
              {localizedDescription}{' '}
              {!localizedInfoMessage && linkExternalProps && (
                <LearnAboutSimSwapRiskLink />
              )}
            </p>
          )}
        </div>
    ...
    <SubRow
      ...
      {...(hasPhoneNumber && !onDeleteClick
        ? {
            ...
            // (i.e. when the user has no other backup method)
      ...
      {...((!hasPhoneNumber || (hasPhoneNumber && onDeleteClick)) && {
        ...
        // are available backup methods (description is intended to allow for comparison of the two methods)
        localizedDescription: ftlMsgResolver.getMsg(
