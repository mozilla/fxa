...
if [[ ! -e config/secret-key.json ]]; then
  node -r ts-node/register/transpile-only -r tsconfig-paths/register  ./scripts/gen_keys.js
fi

if [[ ! -e config/vapid-keys.json ]]; then
  node -r ts-node/register/transpile-only -r tsconfig-paths/register  ./scripts/gen_vapid_keys.js
fi
