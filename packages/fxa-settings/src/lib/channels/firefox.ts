...
export class Firefox extends EventTarget {
  ...
  /**
   * Sends an fxa_status and returns the whole response.
   *
   * Use this instead of `requestSignedInUser` when the caller also needs the
   * browser capabilities. Reading them from the awaited response keeps them in
   * step with the signed-in user, which a separate capability source cannot
   * guarantee. Resolves undefined when the browser does not answer in time.
   */
  async requestFxAStatus(
    context: string,
    isPairing: boolean,
    service: string
  ): Promise<undefined | FxAStatusResponse> {
    let timeoutId: number;
    return Promise.race<undefined | FxAStatusResponse>([
      new Promise<undefined | FxAStatusResponse>((resolve) => {
        const handleFxAStatusEvent = (event: any) => {
          ...
          resolve(event.detail as FxAStatusResponse);
        };
  ...
  async requestSignedInUser(
    ...
    const status = await this.requestFxAStatus(context, isPairing, service);
    return status?.signedInUser;
  }
}
