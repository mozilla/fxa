...
describe('/account/device', () => {
  ...
  it('identical data', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      ...
      const args = mockDevices.isSpuriousUpdate.mock.calls[0];
  ...
  it('different data', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      ...
      const args = mockDevices.upsert.mock.calls[0];
  ...
  it('with no id in payload', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      expect(mockDevices.upsert).toHaveBeenCalledTimes(1);
      const args = mockDevices.upsert.mock.calls[0];
      expect(args[2].id).toBe(mockRequest.auth.credentials.deviceId);
    });
  });
  ...
  it('removes the push endpoint expired flag on callback URL update', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      expect(mockDevices.upsert).toHaveBeenCalledTimes(1);
      expect(mockDevices.upsert.mock.calls[0][2].pushEndpointExpired).toBe(
        false
      );
    });
  });
  ...
  it('all devices', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      return sendPushPromise.then(() => {
        ...
        const args = mockPush.sendPush.mock.calls[0];
  ...
  it('specific devices', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      return sendPushPromise.then(() => {
        expect(mockCustoms.checkAuthenticated).toHaveBeenCalledTimes(1);
        expect(mockPush.sendPush).toHaveBeenCalledTimes(1);
        let args = mockPush.sendPush.mock.calls[0];
        expect(args.length).toBe(4);
        expect(args[0]).toBe(uid);
        expect(Array.isArray(args[1])).toBeTruthy();
        expect(args[2]).toBe('devicesNotify');
...
describe('/account/devices/invoke_command', () => {
  ...
  it('omits sender field when deviceId is null/undefined', () => {
    ...
    return runTest(route, mockRequest).then(() => {
      ...
      expect(mockLog.warn.mock.calls[0][0]).toBe(
        'device.command.senderDeviceIdMissing'
      );
      const warningArgs = mockLog.warn.mock.calls[0][1];
      ...
      expect(mockLog.info).toHaveBeenCalledTimes(2);
      const invokedMetrics = mockLog.info.mock.calls[0][1];
...
describe('/account/device/destroy', () => {
  ...
  it('should destory the device record', () => {
    ...
    return runTest(route, mockRequest, () => {
      ...
      expect(mockDevices.destroy.mock.calls[0][0]).toBe(mockRequest);
      expect(mockDevices.destroy.mock.calls[0][1]).toBe(deviceId);
    });
  });
});
...
describe('/account/devices', () => {
  it('should return the devices list (translated)', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      ...
      expect(mockDevices.synthesizeName.mock.calls[0].length).toBe(1);
      expect(mockDevices.synthesizeName.mock.calls[0][0]).toBe(unnamedDevice);
    });
  });
