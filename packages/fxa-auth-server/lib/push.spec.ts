...
describe('push', () => {
  ...
  it("sendPush doesn't push to ios devices if it is triggered with an unsupported command", async () => {
    ...
    expect(mockSendNotification).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ endpoint: mockDevices[0].pushCallback }),
      expect.anything(),
      expect.anything()
    );
    expect(mockSendNotification).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ endpoint: mockDevices[1].pushCallback }),
      expect.anything(),
      expect.anything()
    );
  });
  ...
  it('sendPush pushes to all ios devices if it is triggered with a "commands received" command', async () => {
    ...
    expect(mockSendNotification).toHaveBeenNthCalledWith(
      3,
      expect.objectContaining({ endpoint: mockDevices[2].pushCallback }),
      expect.anything(),
      expect.anything()
    );
  });
