...
export class CartService {
  ...
  async pollCart(cartId: string): Promise<PollCartResponse> {
    ...
    if (!cart) {
      throw new CartNotFoundError(
        cartId,
        new Error(`Cart ${cartId} not found`)
      );
    } else if (
  ...
  async finalizeProcessingCart(cartId: string): Promise<void> {
    ...
    if (cart.state === CartState.FAIL) {
      return this.finalizeCartWithError(cartId, CartErrorReasonId.BASIC_ERROR);
    }
    ...
    await Promise.all([
      this.checkoutService.postPaySteps(cart, subscription),
