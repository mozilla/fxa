...
describe('DomainBlocklistController', () => {
  ...
  describe('sync', () => {
    ...
    it('counts and warns about the entries it skipped', async () => {
      fetchMock.mockResolvedValue(
        mockResponse(['evil.com', 'not a domain'].join('\n'))
      );
      ...
      expect(logger.warn).toHaveBeenCalledWith('domainBlocklist.sync.skipped', {
        user: 'admin@example.com',
        skipped: 1,
      });
    });
    ...
    it('throws when the url carries credentials', async () => {
      await expect(
        controller.sync(
          'https://user:pass@lists.example.com/list.conf',
          'admin@example.com'
        )
      ).rejects.toThrow(
        'url must not carry credentials, a query string, or a fragment'
      );
      expect(fetchMock).not.toHaveBeenCalled();
    });

    it('throws when the url carries a query string', async () => {
      await expect(
        controller.sync(`${SYNC_URL}?token=secret`, 'admin@example.com')
      ).rejects.toThrow(
        'url must not carry credentials, a query string, or a fragment'
      );
      expect(fetchMock).not.toHaveBeenCalled();
    ...
    it('throws when the url carries a fragment', async () => {
      await expect(
        controller.sync(`${SYNC_URL}#secret`, 'admin@example.com')
      ).rejects.toThrow(
        'url must not carry credentials, a query string, or a fragment'
      );
      expect(fetchMock).not.toHaveBeenCalled();
    ...
    it('throws when the list exceeds the entry cap', async () => {
      fetchMock.mockResolvedValue(mockResponse('a.example\n'.repeat(250_001)));

      await expect(
        controller.sync(SYNC_URL, 'admin@example.com')
      ).rejects.toThrow('List has more than 250000 entries');
      expect(DomainBlocklist.addMany).not.toHaveBeenCalled();
    });
