  ...
  protected _getConnection() {
    ...
    return new Promise(function (resolve, reject) {
      pool.getConnection(function (err: any, conn: any) {
        ...
        if (err) {
          log?.error('MysqlStoreShared', {
            msg: 'MysqlStoreShared: Failed to get connection.',
            err,
          });
          return reject(err);
        }
        ...
        const query = (sql: any) =>
          new Promise<any>((resolve, reject) => {
            conn.query(sql, (err: any, result: any) => {
              if (err) {
                log?.error('MysqlStoreShared', {
                  msg: `MysqlStoreShared: Mysql connection init query failed`,
                  err,
                  uid,
        ...
        return resolve(
          (async () => {
            try {
              ...
              log?.error('MysqlStoreShared', {
                msg: 'MysqlStoreShared: Error initializing connection.',
                err,
                uid,
              });
              ...
              if (conn) {
                log?.info('MysqlStoreShared', {
                  msg: 'MysqlStoreShared: Release connection after DB initialization fails.',
                  uid,
                });
