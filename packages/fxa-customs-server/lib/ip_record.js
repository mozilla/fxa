...
module.exports = function (BLOCK_INTERVAL_MS, IP_RATE_LIMIT_INTERVAL_MS, IP_RATE_LIMIT_BAN_DURATION_MS, MAX_BAD_LOGINS_PER_IP, BAD_LOGIN_ERRNO_WEIGHTS, MAX_ACCOUNT_STATUS_CHECK, now) {
  ...
  IpRecord.prototype.isOverBadLogins = function () {
    ...
    this.lf.forEach(function(info) {
      var incr = BAD_LOGIN_ERRNO_WEIGHTS[info.e] || 1
    ...
    return total > MAX_BAD_LOGINS_PER_IP
  }
  ...
  IpRecord.prototype.trimBadLogins = function (now) {
    this.lf = this._trim(now, this.lf, MAX_BAD_LOGINS_PER_IP)
  }
  ...
  IpRecord.prototype.isOverAccountStatusCheck = function () {
    ...
    return count > MAX_ACCOUNT_STATUS_CHECK
  }
  ...
  IpRecord.prototype.trimAccountStatus = function (now) {
    this.as = this._trim(now, this.as, MAX_ACCOUNT_STATUS_CHECK)
  }
  ...
  IpRecord.prototype._trim = function (now, items, maxUnique) {
    ...
    var seen = {}
    var item = items[i]
    while (item.t > (now - IP_RATE_LIMIT_INTERVAL_MS) && n <= maxUnique) {
      if (!(item.u in seen)) {
        seen[item.u] = true
        n++
      }
