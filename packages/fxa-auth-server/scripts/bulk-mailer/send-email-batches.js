 ...
 */
module.exports = async function(userRecordBatches, batchDelayMS, sendEmail, log, isTest) {
  let successCount = 0
  let errorCount = 0
  ...
  await P.each(userRecordBatches, (currentBatch, index) => {
    return sendEmailBatch(currentBatch, sendEmail, log)
      .then(result => {
    ...
    logComplete(log, successCount, errorCount, totalCount - successCount - errorCount);
  })
}
