...
describe('deauthorizeAccountAuthorizations', () => {
  ...
  it('accepts rows in the shape the disconnect pass sends', async () => {
    // Production passes a hex clientId and a numeric lastAuthorizedTosAt, not
    // the raw Buffer and string the read returns.
    const id = track(newUid());
    await seed({ uid: id, scope: VPN_SCOPE, service: 'vpn', now: T0 });
    const rows = (await db.listAccountConsentsByUid(id)).map(
      (r: ConsentRow) => ({
        scope: r.scope,
        service: r.service,
        clientId: r.clientId.toString('hex'),
...
describe('token exchange after deauthorization', () => {
  ...
  it('still allows the exchange while another client holds an active row', async () => {
    ...
    const desktopRow = rows.find(
      (r: ConsentRow) => r.clientId.toString('hex') === DESKTOP
    );
    await db.deauthorizeAccountAuthorizations(
      id,
      [desktopRow],
...
describe('#integration - allowlist downstream token-exchange consequences', () => {
  ...
  it('getRefreshTokenScopesByUid returns each token’s client and a ScopeSet', async () => {
    const tokens = await db.getRefreshTokenScopesByUid(testClient.uid);

    expect(tokens).toHaveLength(1);
    expect(tokens[0].clientId.toString('hex')).toBe(IOS_REAL);
    expect(tokens[0].scope.contains(OLDSYNC_SCOPE)).toBe(true);
    expect(tokens[0].scope.contains(VPN_SCOPE)).toBe(false);
  });
...
describe('#integration - lifecycle: account deletion vs connected-services deauthorize', () => {
  ...
  it('deauthorizing via attached_client/destroy (connected services) keeps the row and its ToS history', async () => {
    ...
    expect(
      after.map((r: ConsentRow) => Number(r.firstAuthorizedTosAt))
    ).toEqual(before.map((r: ConsentRow) => Number(r.firstAuthorizedTosAt)));
    // The E2E client held only an authorization code, never a refresh token,
    // so nothing sustains its rows once it is disconnected.
    expect(after.every((r: ConsentRow) => r.deauthorizedAt !== null)).toBe(
      true
    );
  });
});
