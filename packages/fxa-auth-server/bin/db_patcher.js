...
var log = require('../log')(config.log.level, 'db-patcher')
...
// only createSchema and patch for 'mysql'
if (config.db.backend !== 'mysql') {
  log.info('Nothing to patch for ' + config.db.backend + ' backend')
  process.exit()
}

var options = config.mysql
if ( !options.createSchema ) {
  log.info('Config option createSchema is false, not patching')
  process.exit()
}
