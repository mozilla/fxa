...
[{}, { v2: true }].forEach((options) => {
  describe(
    '#integration - remote account create ' + (options.v2 ? ' V2' : ''),
    function () {
      this.timeout(30000);
      let server;
      before(async () => {
      ...
      it('create account with service identifier and resume', () => {
        ...
        return Client.create(config.publicUrl, email, password, {
          ...
          .then((emailData) => {
            console.log('!!! emailData', emailData.headers);
      ...
      it('signup with same email, different case', () => {
        ...
        return Client.createAndVerify(
          ...
          .then((c) => {
            return Client.create(config.publicUrl, email2, password, options);
          })
