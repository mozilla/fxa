...
export class MysqlStoreShared {
  ...
  constructor(
    ...
    this._pool.on('enqueue', () => {
      log?.debug('mysql.FXA-4648', { msg: 'on enqueue', uid: this._uid });
      this.metrics?.increment('mysql.connection');
      this.events?.onPoolEnqueue();
    });

    this._pool.on('connection', (connection: any) => {
      log?.debug('mysql.FXA-4648', { msg: 'on enqueue', ...getPoolStats() });
      this.metrics?.increment('mysql.connection', {
    ...
    log?.info('mysql.FXA-4648', {
      msg: 'Creating MysqlStoreShared.',
      ...getPoolStats(),
    });
    log?.trace('mysql.FXA-4648', {
      msg: 'Stack Trace for new MysqlStoreShared.',
      ...getPoolStats(),
      stack: Error().stack,
    });

    // if (!log) {
    //   console.trace('mysql.FXA-4648', 'missing logger!')
    // }
  }
