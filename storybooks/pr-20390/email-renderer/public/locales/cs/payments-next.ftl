...
interstitial-offer-error-customer-mismatch-heading = Toto předplatné není spojeno s vaším účtem
interstitial-offer-error-customer-mismatch-message = Ujistěte se, že jste přihlášeni pomocí správného účtu, nebo kontaktujte podporu, pokud potřebujete pomoci.
...
interstitial-offer-error-button-sign-in = Přihlásit se
interstitial-offer-error-button-contact-support = Kontaktovat podporu
...
subscription-management-free-trial-heading = Zkušební verze zdarma
subscription-management-your-free-trials-aria = Vaše bezplatné zkušební verze
# Heading for mobile only quick links menu
subscription-management-jump-to-heading = Přejít na
subscription-management-nav-free-trials = Zkušební verze zdarma
...
cancel-subscription-button-keep-subscription = Ponechat předplatné
...
churn-cancel-flow-button-cancel-subscription = Zrušit předplatné
...
churn-cancel-flow-after = Poté se vaše předplatné automaticky obnoví se standardním poplatkem, dokud ho nezrušíte.
churn-cancel-flow-cancel-success-title = Je nám líto, že odcházíte
# $productName (String) - The name of the product to create subscription, e.g. Mozilla VPN
# $date (Date) - Last day of product access
...
# The sentence before this informs the customer that they will save a discount on their next bill (e.g. You will save 10% on your next charge of $12 to your PayPal payment method on March 6, 2026.)
churn-stay-subscribed-after = Poté se vaše předplatné automaticky obnoví se standardním poplatkem, dokud ho nezrušíte.
...
free-trial-content-charge-info-with-tax-month = Po skončení bezplatné zkušební doby dne { $date } vám bude každý měsíc účtována částka { $amount } + { $tax } (včetně daně).
free-trial-content-charge-info-with-tax-halfyear = Po skončení bezplatné zkušební doby dne { $date } vám bude každých šest měsíců účtována částka { $amount } + { $tax } (včetně daně).
free-trial-content-charge-info-with-tax-year = Po skončení bezplatné zkušební doby dne { $date } vám bude účtována částka { $amount } + { $tax } za rok.
free-trial-content-charge-info-with-tax-default = Po skončení bezplatné zkušební doby dne { $date } vám bude účtována částka { $amount } + { $tax } (včetně daně).

# Charge info strings - no tax, per interval

free-trial-content-charge-info-no-tax-day = Po skončení bezplatné zkušební doby dne { $date } vám bude účtováno { $amount } za den.
free-trial-content-charge-info-no-tax-week = Po skončení bezplatné zkušební doby dne { $date } vám bude účtováno { $amount } za týden.
free-trial-content-charge-info-no-tax-month = Po skončení bezplatné zkušební doby dne { $date } vám bude účtována částka { $amount } měsíčně.
free-trial-content-charge-info-no-tax-halfyear = Po skončení bezplatné zkušební doby dne { $date } vám bude každých šest měsíců účtována částka { $amount }.
free-trial-content-charge-info-no-tax-year = Po skončení bezplatné zkušební doby dne { $date } vám bude účtována částka { $amount } ročně.
free-trial-content-charge-info-no-tax-default = Po skončení bezplatné zkušební doby dne { $date } vám bude účtována částka { $amount }.
free-trial-content-trial-ends = Vaše bezplatná zkušební verze končí dne { $date }. Aktualizujte svůj způsob platby, abyste si přístup zachovali i po skončení bezplatné zkušební verze.
...
free-trial-content-last-bill = Poslední faktura • { $billedOnDate }
...
interstitial-offer-success-cancel-title = Je nám líto, že odcházíte
# $productName (String) - The name of the product to create subscription, e.g. Mozilla VPN
# $date (Date) - Last day of product access
...
interstitial-offer-button-keep-subscription = Ponechat předplatné
...
free-trial-start-title =
    { $trialDayLength ->
        [one] Vyzkoušejte svou { $trialDayLength }denní bezplatnou zkušební verzi
        [few] Vyzkoušejte svou { $trialDayLength }denní bezplatnou zkušební verzi
       *[other] Vyzkoušejte svou { $trialDayLength }denní bezplatnou zkušební verzi
    }
free-trial-success-title =
    { $trialDayLength ->
        [one] Vaše { $trialDayLength }denní bezplatná zkušební verze začala
        [few] Vaše { $trialDayLength }denní bezplatná zkušební verze začala
       *[other] Vaše { $trialDayLength }denní bezplatná zkušební verze začala
    }
...
free-trial-first-charge-title = První platba: { $endDate }
...
next-charge-with-discount-and-tax-card = Při příštím naúčtování částky { $nextInvoiceTotal } + { $taxDue } na kartu končící číslicemi { $last4 } dne { $currentPeriodEnd } ušetříte { $discountPercent } %.
next-charge-with-discount-and-tax-payment-method = Při příštím zúčtování ve výši { $nextInvoiceTotal } + { $taxDue } ušetříte { $discountPercent } % při platbě prostřednictvím platební metody { $paymentMethod } dne { $currentPeriodEnd }.
next-charge-next-charge-with-discount-and-tax = Při příštím vyúčtování ve výši { $nextInvoiceTotal } + { $taxDue } (včetně daně) k datu { $currentPeriodEnd } ušetříte { $discountPercent } %.
next-charge-with-discount-no-tax-card = Při příštím naúčtování částky { $nextInvoiceTotal } na kartu končící číslicemi { $last4 } dne { $currentPeriodEnd } ušetříte { $discountPercent } %.
next-charge-with-discount-no-tax-payment-method = Při příštím zúčtování ve výši { $nextInvoiceTotal }, které proběhne dne { $currentPeriodEnd } prostřednictvím platební metody { $paymentMethod }, ušetříte { $discountPercent } %.
next-charge-with-discount-no-tax = Při příštím vyúčtování ve výši { $nextInvoiceTotal } dne { $currentPeriodEnd } ušetříte { $discountPercent } %.
next-charge-with-tax-card = Vaše další platba ve výši { $nextInvoiceTotal } + { $taxDue } bude stržena z karty končící číslicemi { $last4 } dne { $currentPeriodEnd }.
next-charge-with-tax-payment-method = Vaše další platba ve výši { $nextInvoiceTotal } + { $taxDue } (včetně daně) bude stržena z vašeho platebního prostředku { $paymentMethod } dne { $currentPeriodEnd }.
