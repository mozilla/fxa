...
upgrade-page-acknowledgment-from-trial = 升級後，目前的免費試用將會立刻結束，並從今天開始依照新方案向您收費。
...
diners-logo-alt-text2 = { -brand-diners } 圖示
...
new-account-prepaid-card-free-trial-not-allowed = 新帳號不可使用預付卡進行免費試用。請改用其他付款方式。
...
interstitial-offer-error-customer-mismatch-heading = 此訂閱紀錄並未與您的帳號關聯
interstitial-offer-error-customer-mismatch-message = 請確認您登入的是正確帳號，若需協助，請聯絡支援團隊。
...
interstitial-offer-error-button-sign-in = 登入
interstitial-offer-error-button-contact-support = 聯絡支援團隊
...
checkbox-payment-required-no-charge = 需要設定付款方式，才能啟動免費試用。在 { $endDate } 之前都不會向您收費。
checkbox-confirm-free-trial-with-legal-links = 我授權 { -brand-mozilla } 根據<termsOfServiceLink>服務條款</termsOfServiceLink>與<privacyNoticeLink>隱私權公告</privacyNoticeLink>的內容，在試用期於 { $endDate } 結束之後，對我的付款方式收取此費用，直到我主動取消訂閱為止。
...
free-trial-content-charge-info-no-tax-halfyear = 免費試用於 { $date } 結束後，將向您收取每 6 個月 { $amount }。
free-trial-content-charge-info-no-tax-year = 免費試用於 { $date } 結束後，將向您收取每年 { $amount }。
free-trial-content-charge-info-no-tax-default = 免費試用於 { $date } 結束後，將向您收取 { $amount }。
free-trial-content-trial-ends = 您的免費試用將於 { $date } 結束。請更新付款方式，以在試用結束後還能持續使用。
...
interstitial-offer-button-keep-current-interval-halfyearly = 保持 6 個月訂閱
...
free-trial-start-title = 開始 { $trialDayLength } 天免費試用
free-trial-success-title = 已開始您的 { $trialDayLength } 天免費試用
...
free-trial-start-message-halfyearly = 目前無需付款，將會在免費試用期於 { $endDate } 結束後，向您收取每 6 個月 { $firstPrice }。
...
free-trial-first-charge-message-halfyearly = 將在 { $endDate } 向您收費 { $firstPrice }，然後每 6 個月持續收費直到您主動取消。
...
subscription-content-link-churn-intervention-terms-aria = 檢視折扣碼條款與限制
...
next-charge-with-tax-payment-method = 下期將於 { $currentPeriodEnd } 透過 { $paymentMethod } 收費 { $nextInvoiceTotal } + { $taxDue } 稅。
next-charge-with-tax = 下期將於 { $currentPeriodEnd } 收費 { $nextInvoiceTotal } + { $taxDue } 稅。
next-charge-no-tax-card = 下期將於 { $currentPeriodEnd } 透過卡號末四碼為 { $last4 } 的卡片收費 { $nextInvoiceTotal }。
next-charge-no-tax-payment-method = 下期將於 { $currentPeriodEnd } 透過 { $paymentMethod } 收費 { $nextInvoiceTotal }。
next-charge-no-tax = 下期將於 { $currentPeriodEnd } 收費 { $nextInvoiceTotal }。
