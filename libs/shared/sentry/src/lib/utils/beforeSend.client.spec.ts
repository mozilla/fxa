...
describe('applyCommonTags', () => {
  ...
  it.each([
    { errno: 123, known: true },
    { errno: '123', known: true },
    { errno: 'none', known: false },
    { errno: undefined, known: false },
  ])('tags errno $errno as known error $known', ({ errno, known }) => {
    const event = applyCommonTags(
      { tags: { errno } },
      { name: 'fxa-settings', runtime: 'browser' }
    );

    expect(event.tags[SentryTags.KNOWN_ERROR]).toBe(known);
  });
});
