...
describe('MeteringSweepManager', () => {
  ...
  describe('findActiveMeters', () => {
    it('returns the active meters from the repository', async () => {
      const meters = [scope, { clientId: 'relay', slug: 'bandwidth' }];
      meteringSweepRepository.findActiveMeters.mockResolvedValue(meters);

      await expect(
        manager.findActiveMeters({
          ingestedSince: new Date('2026-05-14T12:00:00.000Z'),
        })
      ).resolves.toEqual(meters);
    });
  });
});
