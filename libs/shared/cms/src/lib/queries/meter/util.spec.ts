...
describe('toStrapiMeter', () => {
  ...
  it('accepts a duration exactly at the retention limit', () => {
    const meter = StrapiMeterRawFactory({
      windowKind: 'sliding',
      windowPeriod: null,
      windowDurationMinutes: METERING_MAX_WINDOW_DURATION_MINUTES,
    });
    expect(toStrapiMeter(meter).window).toEqual({
      kind: 'sliding',
      durationMs: METERING_MAX_WINDOW_DURATION_MINUTES * 60_000,
    });
  });

  it.each(['sliding', 'session'] as const)(
    'rejects a %s meter whose duration outlives event retention',
    (windowKind) => {
      const meter = StrapiMeterRawFactory({
        windowKind,
        windowPeriod: null,
        windowDurationMinutes: METERING_MAX_WINDOW_DURATION_MINUTES + 1,
      });
      expect(() => toStrapiMeter(meter)).toThrow(MeterInvalidWindowError);
    }
  );
