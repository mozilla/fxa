...
describe('DB metrics', () => {
  ...
  it(
    ...
    () => {
      ...
      .then(function (results) {
        assert(results[0][0].count === lastResults[0].count + 1, 'account count was incremented by one')
        assert(results[1][0].count === lastResults[1].count, 'verified account count was not incremented')
        assert(results[2][0].count === lastResults[2].count, '2+ device account count was not incremented')
        assert(results[3][0].count === lastResults[3].count, '3+ device account count was not incremented')
        assert(results[4][0].count === lastResults[4].count, 'mobile device account count was not incremented')
        lastResults = results
      ...
      .then(function (results) {
        assert(results[2][0].count === lastResults[0][0].count, 'account count was not incremented')
        assert(results[3][0].count === lastResults[1][0].count + 1, 'verified account count was incremented by one')
        assert(results[4][0].count === lastResults[2][0].count, '2+ device account count was not incremented')
        assert(results[5][0].count === lastResults[3][0].count, '3+ device account count was not incremented')
        assert(results[6][0].count === lastResults[4][0].count, 'mobile device account count was not incremented')
        lastResults = results
        return P.all([
      ...
      .then(function (results) {
        assert(results[0].count === lastResults[2][0].count - 1, 'account count was decremented by one')
        assert(results[1].count === lastResults[3][0].count - 1, 'verified account count was decremented by one')
        assert(results[2].count === lastResults[4][0].count + 1, '2+ device account count was incremented by one')
        assert(results[3].count === lastResults[5][0].count, '3+ device account count was not incremented')
        assert(results[4].count === lastResults[6][0].count, 'mobile device account count was not incremented')
        lastResults = results
      ...
      .then(function (results) {
        assert(results[0].count === lastResults[2].count, '2+ device account count was not incremented')
        assert(results[1].count === lastResults[3].count + 1, '3+ device account count was incremented by one')
        assert(results[2].count === lastResults[4].count, 'mobile device account count was not incremented')
        lastResults = results
        return P.all([
      ...
      .then(function (results) {
        assert(results[0].count === lastResults[0].count, '2+ device account count was not incremented')
        assert(results[1].count === lastResults[1].count, '3+ device account count was not incremented')
        assert(results[2].count === lastResults[2].count + 1, 'mobile device account count was incremented by one')
        lastResults = results
      })
      .then(function () {
        return P.all([
      ...
      .then(function (results) {
        assert(results[0].count === lastResults[0].count - 1, '2+ device account count was decremented by one')
        assert(results[1].count === lastResults[1].count - 1, '3+ device account count was decremented by one')
        assert(results[2].count === lastResults[2].count - 1, 'mobile device account count was decremented by one')
        return deleteAccount(uid)
      })
      .then(function () {
  ...
  it(
    ...
    () => {
      ...
      .then(function () {
        ...
        assert.equal(queries[0].sql, 'SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED', 'first query had correct SQL')
        assert.equal(queries[1].sql, 'START TRANSACTION', 'second query had correct SQL')
        assert.equal(queries[2].sql, metrics.countAccounts, 'third query had correct SQL')
        assert.equal(queries[3].sql, metrics.countVerifiedAccounts, 'fourth query had correct SQL')
        assert.equal(queries[4].sql, metrics.countAccountsWithTwoOrMoreDevices, 'fifth query had correct SQL')
        ...
        assert.equal(data.total_accounts, 1, 'log file total_accounts property was correct')
        assert.equal(data.total_verified_accounts, 2, 'log file total_verified_accounts property was correct')
        ...
        assert.equal(time.getUTCDate(), 10, 'log file time property had correct date')
        assert.equal(time.getUTCHours(), 0, 'log file time property had correct hour')
        assert.equal(time.getUTCMinutes(), 0, 'log file time property had correct minute')
        assert.equal(time.getUTCSeconds(), 0, 'log file time property had correct second')
        assert.equal(time.getUTCMilliseconds(), 0, 'log file time property had correct millisecond')
