...
-product-mozilla-account = Mozilla mba’ete
-product-mozilla-accounts = Mozilla mba’etekuéra
...
link-sr-new-window = Oñembojuruja ovetã pyahúpe
...
button-passkey-signin = Eikekuaa ñe’ẽñemi reheve
button-passkey-signin-loading = Eikekuaa tekorosãme…
...
cs-scope-firefox-vpn = { -brand-firefox } VPN juajupyre
...
passkey-sub-row-created-date = Heñói: { $createdDate }
...
auth-error-225 = Ñe’ẽ ñemi mbokuatiapyre
...
pair-auth-allow-refuse-device-link = Nandéiramo, <a>emoambue ne ñe’ẽñemi</a>
...
pair-auth-complete-send-tab-heading = Emondokuaáma ndahetái tendayke
...
pair-success-tab-close-message = Ko tendayke { -brand-firefox } ombotýta ijeheguiete.
...
signin-passwordless-code-instruction =
    { $expirationMinutes ->
        [one] Ehai ayvu ohóva <email>{ $email }</email>-pe { NUMBER($expirationMinutes) } aravo’ieta pa’ũme.
       *[other] Ehai ayvu ohóva <email>{ $email }</email>-pe { NUMBER($expirationMinutes) } aravo’ietaite pa’ũme.
    }
