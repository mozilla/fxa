...
// Longest we hold a navigation open for a ping to flush. `GleanMetrics.isDone()`
// polls an unbounded queue and never rejects, so on its own it can leave the user
// stranded on this screen with neither the app nor the store.
const PING_FLUSH_MS = 250;

function flushThenNavigate(url: string) {
  Promise.race([
    GleanMetrics.isDone(),
...
const PocPairInit = () => {
  ...
  useEffect(() => {
    ...
    // Deliberately not `flushThenNavigate` here, unlike the other two
