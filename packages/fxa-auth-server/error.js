...
module.exports = function(log) {
  ...
  // Make a new Error object that inherits all properties of Boom.
  // XXX TODO: please somebody teach me the correct JS idiom here...
  function Error() {}
  Error.prototype = Boom
  Error = new Error()
    ...
    if (typeof object.errno === 'undefined') {
      if (object.code === 401) {
        ...
        if (object.message === 'Unknown credentials') {
          object = Error.invalidToken().response.payload
        } else if (object.message === 'Invalid credentials') {
          object = Error.invalidToken().response.payload
        } else if (object.message === 'Stale timestamp') {
          object = Error.invalidTimestamp().response.payload
        } else if (object.message === 'Invalid nonce') {
          object = Error.invalidNonce().response.payload
        } else {
          object = Error.invalidSignature(object.message).response.payload
        }
      }
      else if (object.code === 400) {
        if (TOO_LARGE.test(object.message)) {
          object = Error.requestBodyTooLarge().response.payload
        }
      }
    }
  ...
  Error.accountExists = function (email) {
    return Error.wrap({
      code: 400,
      errno: 101,
  ...
  Error.unknownAccount = function (email) {
    return Error.wrap({
      code: 400,
      errno: 102,
  ...
  Error.incorrectPassword = function () {
    return Error.wrap({
      code: 400,
      errno: 103,
  ...
  Error.unverifiedAccount = function () {
    return Error.wrap({
      code: 400,
      errno: 104,
  ...
  Error.invalidVerificationCode = function (details) {
    return Error.wrap(Hoek.merge({
      code: 400,
      errno: 105,
  ...
  Error.invalidRequestBody = function () {
    return Error.wrap({
      code: 400,
      errno: 106,
  ...
  Error.invalidRequestParameter = function (param) {
    return Error.wrap({
      code: 400,
      errno: 107,
  ...
  Error.missingRequestParameter = function (param) {
    return Error.wrap({
      code: 400,
      errno: 108,
  ...
  Error.invalidSignature = function (message) {
    return Error.wrap({
      code: 401,
      errno: 109,
  ...
  Error.invalidTimestamp = function () {
    return Error.wrap({
      code: 401,
      errno: 111,
  ...
  Error.missingContentLength = function () {
    return Error.wrap({
      code: 411,
      errno: 112,
  ...
  Error.requestBodyTooLarge = function () {
    return Error.wrap({
      code: 413,
      errno: 113,
  ...
  Error.tooManyRequests = function () {
    return Error.wrap({
      code: 429,
      errno: 114,
  ...
  Error.serviceUnavailable = function () {
    return Error.wrap({
      code: 503,
      errno: 201,
