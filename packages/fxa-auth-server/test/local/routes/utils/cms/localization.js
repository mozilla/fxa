...
describe('CMSLocalization', () => {
  ...
  describe('strapiToFtl', () => {
    it('converts Strapi data to FTL format', () => {
      ...
      // With fxa-prefixed hash IDs, we expect fxa- + hash patterns
      assert.match(result, /fxa-[a-f0-9]{8} = Enter your password/);
      assert.match(result, /fxa-[a-f0-9]{8} = to sign in to Firefox and start syncing/);
      assert.match(result, /fxa-[a-f0-9]{8} = Welcome to Firefox Sync/);
      assert.match(result, /fxa-[a-f0-9]{8} = Sync your passwords, tabs, and bookmarks/);
    });
    ...
    it('filters out non-string fields', () => {
      ...
      // With fxa-prefixed hash IDs, we expect fxa- + hash patterns
      assert.match(result, /fxa-[a-f0-9]{8} = Enter your password/);
      assert.match(result, /fxa-[a-f0-9]{8} = to sign in/);
  ...
  describe('mergeConfigs', () => {
    it('applies translation when hash matches', async () => {
      ...
      const hashMatch = generatedFtl.match(/(fxa-[a-f0-9]{8}) = Enter your password/);
