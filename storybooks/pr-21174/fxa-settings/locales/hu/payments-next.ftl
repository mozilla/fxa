...
diners-logo-alt-text2 = { -brand-diners } logó
...
new-account-prepaid-card-free-trial-not-allowed = Előre feltöltött kártyákkal nem lehet ingyenes próbaidőszakot kezdeni új fiók estén. Próbáljon meg egy másik fizetési módot.
...
next-payment-confirm-with-legal-links-static-3 = Felhatalmazom a { -brand-mozilla(ending: "accented") }t, hogy terhelje meg a megjelenített összeggel a fizetési módomat a <termsOfServiceLink>Szolgáltatási feltételek</termsOfServiceLink> és az <privacyNoticeLink>Adatvédelmi nyilatkozat</privacyNoticeLink> szerint, amíg le nem mondom az előfizetést.

## $endDate (Date) - The end date of the free trial

checkbox-payment-required-no-charge = Az ingyenes próbaidőszak megkezdéséhez fizetési mód szükséges. Eddig nem lesz megterhelve: { $endDate }.
checkbox-confirm-free-trial-with-legal-links = Felhatalmazom a { -brand-mozilla(ending: "accented") }t, hogy a próbaidőszak vége ({ $endDate }) után terhelje meg a megjelenített összeggel a fizetési módomat a <termsOfServiceLink>Szolgáltatási feltételek</termsOfServiceLink> és az <privacyNoticeLink>Adatvédelmi nyilatkozat</privacyNoticeLink> szerint, amíg le nem mondom az előfizetést.
...
churn-cancel-flow-success-message = Az előfizetése folytatódni fog, és { $discountPercent }%-ot takarít meg a következő számlájából.
# $productName (String) - The name of the product to create subscription, e.g. Mozilla VPN
churn-cancel-flow-thanks-valued-subscriber = Köszönjük, hogy a(z) { $productName } terméket használja.
...
churn-cancel-flow-button-manage-subscriptions = Előfizetések kezelése
...
free-trial-content-last-bill = Utolsó számla • { $billedOnDate }
free-trial-content-last-bill-with-tax = { $invoiceTotal } + { $taxDue } adó
...
free-trial-first-charge-title = Első terhelés: { $endDate }
...
churn-error-page-message-customer-mismatch = Ez a kupon egy másik előfizetéshez lett kiadva, és csak az eredeti címzett válthatja be.
...
next-charge-with-discount-no-tax-card = { $discountPercent }%-ot fog megtakarítani, amikor { $nextInvoiceTotal } összeget fizet a(z) { $last4 } végű kártyájával ekkor: { $currentPeriodEnd }.
next-charge-with-discount-no-tax-payment-method = { $discountPercent }%-ot fog megtakarítani, amikor { $nextInvoiceTotal } összeget fizet a(z) { $paymentMethod } segítségével ekkor: { $currentPeriodEnd }.
next-charge-with-discount-no-tax = { $discountPercent }%-ot fog megtakarítani, amikor { $nextInvoiceTotal } összeget fizet ekkor: { $currentPeriodEnd }.
...
next-charge-with-tax-payment-method = A következő terhelés { $nextInvoiceTotal } + { $taxDue } adó lesz a(z) { $paymentMethod } segítségével ekkor: { $currentPeriodEnd }.
next-charge-with-tax = A következő terhelés { $nextInvoiceTotal } + { $taxDue } adó lesz ekkor: { $currentPeriodEnd }.
next-charge-no-tax-card = A(z) { $last4 } végű kártya következő terhelése { $nextInvoiceTotal } lesz, ekkor: { $currentPeriodEnd }.
next-charge-no-tax-payment-method = A következő terhelés { $nextInvoiceTotal } lesz a(z) { $paymentMethod } segítségével ekkor: { $currentPeriodEnd }.
