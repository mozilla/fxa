...
function buildSubscription(
  overrides: Partial<Parameters<typeof SubscriptionContent>[0]['subscription']>
) {
  return {
    id: 'sub_abc123',
...
describe('SubscriptionContent', () => {
  describe('active subscription', () => {
    ...
    it('renders plan interval — monthly', async () => {
      const subscription = buildSubscription({ interval: 'monthly' });
    ...
    it('renders plan interval — yearly', async () => {
      const subscription = buildSubscription({ interval: 'yearly' });
  ...
  describe('active subscription — edge cases', () => {
    it('renders without invoice URL when currentInvoiceUrl is null', async () => {
      ...
      expect(screen.queryByTestId('link-external-view-invoice')).not.toBeInTheDocument();
    });
