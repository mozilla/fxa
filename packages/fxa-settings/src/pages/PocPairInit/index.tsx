...
const PocPairInit = () => {
  ...
  useEffect(() => {
    ...
    if (isFirefox) {
      ...
      // drop this one.
      flushPingsThen(() => window.location.assign(target));
      return;
    }
    ...
    // Deliberately not `flushPingsThen` here, unlike the other two
  ...
  const armStoreFallback = () => {
    ...
    const goToStore = (reason: 'timeout' | 'focus_grace') => {
      ...
      // navigating in the same tick would drop it. The wait opens a window in
      // which the app can still take the foreground; the listeners that would
      // have recorded that are already torn down, so ask the document directly
      // and leave a hidden page where it is.
      flushPingsThen(
