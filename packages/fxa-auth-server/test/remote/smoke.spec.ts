...
/**
 * Minimal smoke test to validate the Jest integration test infrastructure.
 * This test verifies:
 * 1. Test server starts on a dynamic port
 * 2. HTTP requests work against the server
...
describe('#integration - smoke test', () => {
  ...
  beforeAll(async () => {
    ...
    console.log(`Test server started on ${server.publicUrl}`);
  });
  ...
  it('server responds to API endpoint', async () => {
    ...
    console.log('account/status response:', response.status, body);
    // Should get 200 with exists: false for non-existent account
