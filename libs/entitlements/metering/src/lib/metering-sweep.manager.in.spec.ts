...
import { WATERMARKS_TABLE } from './metering.constants';
import {
  anchoredClock,
  DAY_MS,
  HOUR_MS,
...
// This suite needs a real ClickHouse. `yarn start infrastructure` runs one
// locally, but CI does not, so skip when CI is set unless explicitly forced on.
const describeClickHouse =
  process.env['CI'] && !process.env['CLICKHOUSE_INTEGRATION_TESTS']
    ? describe.skip
    : describe;

describeClickHouse('MeteringSweepManager against a real ClickHouse', () => {
  ...
  describe('watermark round-trip', () => {
    ...
    async function advanceAll(offsets: number[]): Promise<void> {
    ...
    it('returns the highest watermark when slices share an updated_at', async () => {
      await advanceAll([-3 * HOUR_MS, -2 * HOUR_MS, -1 * HOUR_MS]);

      await expect(
        manager.findWatermark({ clientId: CLIENT_ID, slug: SLUG })
      ).resolves.toEqual(at(-1 * HOUR_MS));
    });

    it('returns the highest tied watermark before the parts merge regardless of insert order', async () => {
      await advanceAll([-1 * HOUR_MS, -2 * HOUR_MS, -3 * HOUR_MS]);
    ...
    it('still returns the highest tied watermark after the parts merge when written ascending', async () => {
      await advanceAll([-3 * HOUR_MS, -2 * HOUR_MS, -1 * HOUR_MS]);
      await optimizeFinal(client, WATERMARKS_TABLE);
    ...
    it('keeps the last inserted tied watermark after the parts merge when written descending', async () => {
      await advanceAll([-1 * HOUR_MS, -2 * HOUR_MS, -3 * HOUR_MS]);
      await optimizeFinal(client, WATERMARKS_TABLE);

      await expect(
        manager.findWatermark({ clientId: CLIENT_ID, slug: SLUG })
      ).resolves.toEqual(at(-3 * HOUR_MS));
    });

    it('honors a rewind written with a newer updated_at', async () => {
      await manager.advanceWatermark({
        clientId: CLIENT_ID,
        slug: SLUG,
        watermark: at(-1 * HOUR_MS),
        updatedAt: at(0),
      });
      await manager.advanceWatermark({
        clientId: CLIENT_ID,
        slug: SLUG,
        watermark: at(-3 * HOUR_MS),
        updatedAt: at(60_000),
      });
  ...
  describe('activity gate on ingested_at', () => {
    ...
    it('excludes a subject ingested at or after the slice end', async () => {
      await events.insertEvents([
        {
          ...event('late', 'user-1', 90, at(-1.5 * DAY_MS)),
          ingestedAt: at(-60000),
  ...
  describe('session start detection', () => {
    ...
    it('opens a new session for an event exactly at the previous expiry', async () => {
      await manager.recordSessionStarts([
        {
          clientId: CLIENT_ID,
          slug: SLUG,
      ...
      await events.insertEvents([
        {
          ...event('e1', 'user-1', 5, at(-6 * HOUR_MS + FIVE_HOURS_MS)),
          ingestedAt: at(-120_000),
        },
      ]);
      ...
      await expect(
        ...
        expect.objectContaining({
          sessionStart: at(-6 * HOUR_MS + FIVE_HOURS_MS),
        }),
      ]);
    });

    it('opens a session at the first post-expiry event even when an in-session event was ingested alongside it', async () => {
      await manager.recordSessionStarts([
        {
          clientId: CLIENT_ID,
          slug: SLUG,
      ...
      await events.insertEvents([
        {
          ...event('in-session', 'user-1', 5, at(-70 * 60_000)),
          ingestedAt: at(-120_000),
        },
        {
          ...event('post-expiry', 'user-1', 5, at(-50 * 60_000)),
          ingestedAt: at(-100_000),
        },
      ]);
      ...
      await expect(
        ...
        expect.objectContaining({
          subject: 'user-1',
          sessionStart: at(-50 * 60_000),
        }),
      ]);
    });

    it('ignores events ingested at or after ingestedUntil', async () => {
      await events.insertEvents([
        { ...event('e1', 'user-1', 5, at(-240000)), ingestedAt: at(-60_000) },
      ]);

      await expect(
        manager.findNewSessionStarts(
          detectParams({ ingestedUntil: at(-90_000) })
        )
      ).resolves.toEqual([]);
    });
  ...
  describe('findActiveMeters', () => {
    it('returns nothing when there are no events or watermarks', async () => {
      await expect(
        manager.findActiveMeters({ ingestedSince: at(-DAY_MS) })
      ).resolves.toEqual([]);
    });

    it('lists meters with recent events and meters with a watermark, sorted', async () => {
      await events.insertEvents([
        event('e1', 'user-1', 1, at(-HOUR_MS)),
        {
          ...event('e2', 'user-1', 1, at(-HOUR_MS)),
          clientId: 'relay',
          slug: 'bandwidth',
        },
        {
          ...event('e3', 'user-1', 1, at(-2 * DAY_MS)),
          clientId: 'stale',
          slug: 'meter',
        },
      ]);
