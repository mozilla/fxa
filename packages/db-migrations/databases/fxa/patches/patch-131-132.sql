...
INSERT IGNORE INTO dbMetadata (name, value) VALUES ('lastPrunedAt', '0');
...
UPDATE dbMetadata SET value = '0' WHERE name = 'lastPrunedAt' AND value = '';
...
CREATE PROCEDURE `prune_8` (
  ...
  IN `pruneInterval` BIGINT UNSIGNED,
...
BEGIN
  ...
  IF @lockAcquired THEN
    ...
    select @lastPrunedAt := value from dbMetadata where name = 'lastPrunedAt';

    -- Respect pruning interval
    IF curTime > @lastPrunedAt + pruneInterval THEN

      UPDATE dbMetadata
      SET value = curTime
      where name = 'lastPrunedAt';
