...
export class PairingSupplicantIntegration extends OAuthWebIntegration {
  ...
  private handleAuthAuthorize = (event: Event) => {
    ...
    // code is required and must be a hex string; state must match original.
    if (!data?.code || !/^[0-9a-f]+$/i.test(data.code)) {
      this._error = new Error(
        'Authority approved without providing a valid OAuth code'
      );
      this.setState(SupplicantState.Failed);
      this.onError?.(this._error);
      return;
    ...
    if (expectedState && data.state !== expectedState) {
      this._error = new Error('OAuth state mismatch');
      this.setState(SupplicantState.Failed);
      this.onError?.(this._error);
      return;
    }
  ...
  private getOAuthParams(): {
    ...
    if (!clientId || !codeChallenge || !codeChallengeMethod || !keysJwk || !state || !rawScope) {
    ...
    const scope = [...new Set(rawScope.replace(/\+/g, ' ').trim().split(/\s+/))]
      .join(' ');
