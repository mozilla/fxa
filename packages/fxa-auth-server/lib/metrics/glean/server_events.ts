...
class EventsServerEventLogger {
   ...
   * A device pairing authorization code was minted for a mobile client. In the pair2 flow this is only reachable once both devices have confirmed, so it counts confirmed pairings. Two caveats. Legacy pairing reaches the same endpoint earlier — when the desktop user approves, possibly before the phone has — so its rows mean "authority approved" rather than "both confirmed", and the two cannot be told apart server side. Legacy also supports a desktop supplicant, whose client_id falls outside the mobile set and so is not counted at all.
   * Event is logged using internal mozlog logger.
   *
   * @param {string} user_agent - The user agent.
   * @param {string} ip_address - The IP address. Will be used to decode Geo
