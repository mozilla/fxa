...
import { AppSettingsContext } from '../../../models';
...
describe('Avatar', () => {
  it('renders default avatar with expected attributes', () => {
    render(
      <AppSettingsContext.Provider value={{ account: MOCK_AVATAR_DEFAULT }}>
        <Avatar />
  ...
  it('renders the avatar with expected attributes', () => {
    render(
      <AppSettingsContext.Provider value={{ account: MOCK_AVATAR_NON_DEFAULT }}>
        <Avatar />
      </AppSettingsContext.Provider>
    );
