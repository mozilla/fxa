...
function isPairingAuthorization(request, clientId) {
  ...
  // `deviceType` is null only for a desktop UA — but not for the Sync-style
  // `Firefox-<os>-FxA*` agents, which `parseToScalars` sends through
  // `marshallDeviceType`, defaulting them to 'mobile' and so to not-a-pairing.
