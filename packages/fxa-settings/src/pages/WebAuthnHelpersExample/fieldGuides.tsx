...
export const CREATION_FIELDS: FieldSpec[] = [
  ...
  {
    path: 'user.id',
    desc: 'User handle (base64url). Must be unique and stable per user account; not displayed to the user.',
  },
  {
    path: 'timeout',
  ...
  },
  {
    ...
    desc: 'Credential IDs the authenticator must not re-register. Add a registered credential ID here, then re-register the same passkey to trigger InvalidStateError.',
  },
  {
...
export const REQUEST_FIELDS: FieldSpec[] = [
  ...
  },
  {
    ...
    desc: 'Credential IDs to target. Omit or leave as an empty array for discoverable / usernameless flow — the browser shows all passkeys for this domain.',
  },
  {
    path: 'extensions.prf.eval.first',
