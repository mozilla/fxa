...
describe('PageRecoveryPhoneSetup', () => {
  ...
  it('add recovery phone with successful submission renders step 2', async () => {
    ...
    await waitFor(() =>
      expect(addRecoveryPhone).toHaveBeenCalledWith(phoneNumber)
    );
    expect(
      screen.getByText(/You’ll get a text message from Mozilla/i)
  ...
  it('at step 2, allows code resend', async () => {
    ...
    const resendCodeButton = screen.getByRole('button', {
      name: /Resend code/i,
    });
    user.click(resendCodeButton);
  ...
  it('at step 2, allows code confirm', async () => {
    ...
    await completeStepOne(user);
    await waitFor(() =>
      user.type(screen.getByLabelText(/Enter 6-digit code/i), otpCode)
    );

    expect(confirmRecoveryPhone).toHaveBeenCalledTimes(1);
    expect(confirmRecoveryPhone).toHaveBeenCalledWith(otpCode, phoneNumber);
  });
});
