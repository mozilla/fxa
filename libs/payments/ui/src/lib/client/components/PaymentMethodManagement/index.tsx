...
export function PaymentMethodManagement({
  ...
  onSubmit: (uid: string, confirmationTokenId: string) => any;
  uid?: string;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await onSubmit(uid ?? '', confirmationToken.id);
      router.refresh();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="p-4">
      ...
      {isReady && (
        <h2 className="font-semibold mb-4" id="stripe-payment-management">
          {l10n.getString(
      ...
      <Form.Root ref={formRef} onSubmit={handleSubmit}>
        ...
        <Form.Field name="payment">
          ...
          {error && (
            <Form.Message className="text-red-600 text-sm mt-2 pt-2">
              {error}
            </Form.Message>
          )}
        </Form.Field>
