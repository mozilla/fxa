...
let mockAccountQuickDelete = sinon.fake.resolves(undefined);
let mockAccountTasksDeleteAccount = sinon.fake(async (...args: any[]) => {});
...
const makeRoutes = function (options: any = {}, requireMocks: any = {}) {
  ...
  const logInstance = options.log || mocks.mockLog();
  Container.set(AuthLogger, logInstance);
  ...
  const Password =
    options.Password || require('../crypto/password')(logInstance, config);
  ...
  const signinUtils =
    options.signinUtils ||
    require('./utils/signin')(logInstance, config, customs, db, mailer, cadReminders, glean, statsd);
  if (options.checkPassword) {
    signinUtils.checkPassword = options.checkPassword;
  }
  const push = options.push || require('../push')(logInstance, db, {});
  ...
  const pushbox = options.pushbox || { deleteAccount: sinon.fake.resolves(undefined) };
  const oauthDb = {
  ...
  mockAccountQuickDelete = sinon.fake(async (...args: any[]) => {
    return {};
  });
  ...
  Container.set(BackupCodeManager, {
    getCountForUserId: sinon.stub().resolves(0),
  });
  Container.set(RecoveryPhoneService, {
    hasConfirmed: sinon.stub().resolves(false),
    available: sinon.stub().resolves(false),
  });
...
describe('/account/create', () => {
  ...
  it('should create a sync account', () => {
    ...
    }).finally(() => (Date.now as any).restore());
  });
  ...
  it('should reject creation when email is reserved in Redis', async () => {
    ...
    try {
      ...
      expect(err.errno).toBe(error.ERRNO.VERIFIED_SECONDARY_EMAIL_EXISTS);
  ...
  it('should return an error if email fails to send', () => {
    ...
    return runTest(route, mockRequest).then(
      ...
      (err: any) => {
        ...
        expect(err.output.payload.code).toBe(422);
        expect(err.output.payload.errno).toBe(151);
  ...
  it('should return a bounce error if send fails with one', () => {
    ...
    return runTest(route, mockRequest).then(
      () => { throw new Error('should have failed'); },
      (err: any) => {
        expect(err.message).toBe('Email account hard bounced');
        expect(err.output.payload.code).toBe(400);
        expect(err.output.payload.errno).toBe(134);
  ...
  it('can refuse new account creations for selected OAuth clients', async () => {
    ...
    try {
      ...
      expect(err.errno).toBe(error.ERRNO.DISABLED_CLIENT_ID);
    }
  });
...
describe('/account/stub', () => {
  ...
  it('rejects creating an account with an invalid email domain', async () => {
    ...
    try {
      ...
      expect(err.errno).toBe(error.ERRNO.ACCOUNT_CREATION_REJECTED);
    }
  });
});
...
describe('/account/login', () => {
  ...
  beforeEach(() => {
    ...
    Container.set(CapabilityService, sinon.fake.resolves(undefined));
  ...
  describe('sign-in confirmation', () => {
    ...
    it('should return an error if email fails to send', () => {
      ...
      return runTest(route, mockRequest).then(
        ...
        (err: any) => {
          expect(err.message).toBe('Failed to send email');
          expect(err.output.payload.code).toBe(500);
          expect(err.output.payload.errno).toBe(151);
        ...
        it('without unblock code', () => {
          return runTest(route, mockRequest).then(
            ...
            (err: any) => {
              ...
              expect(err.output.payload.verificationReason).toBe('login');
              expect(mockLog.flowEvent.callCount).toBe(1);
  ...
  it('can refuse new account logins for selected OAuth clients', async () => {
    ...
    try {
      await runTest(innerRoute, innerMockRequest);
      throw new Error('should have errored');
    } catch (err: any) {
      expect(err.output.statusCode).toBe(503);
      expect(err.errno).toBe(error.ERRNO.DISABLED_CLIENT_ID);
  ...
  it('should use RP CMS email content for new login email', () => {
    ...
    // Rebuild route without forcedEmailAddresses so login succeeds without confirmation
    const savedForcedEmails = config.signinConfirmation.forcedEmailAddresses;
    config.signinConfirmation.forcedEmailAddresses = undefined;
    const cmsRoutes = makeRoutes({
      checkPassword: function () {
        return Promise.resolve(true);
      },
      config: config,
    ...
    const cmsRoute = getRoute(cmsRoutes, '/account/login');
    return runTest(cmsRoute, mockRequestWithRpCmsConfig, () => {
      config.signinConfirmation.forcedEmailAddresses = savedForcedEmails;
...
describe('/account/destroy', () => {
  ...
  it('should delete the account and enqueue account task on error', () => {
    ...
    mockAccountQuickDelete = sinon.fake.rejects(new Error('quickDelete failed'));
  ...
  describe('web subscriptions', () => {
    ...
    it('should return formatted Stripe subscriptions when subscriptions are enabled', () => {
      return runTest(buildRoute(), request, (result: any) => {
        sinon.assert.calledOnceWithExactly(logInstance.begin, 'Account.get', request);
        sinon.assert.calledOnceWithExactly(
    ...
    it('should swallow unknownCustomer errors from stripe.customer', () => {
      ...
      return runTest(buildRoute(), request, (result: any) => {
        ...
        expect(logInstance.begin.callCount).toBe(1);
...
describe('/account/email_bounce_status', () => {
  let logInstance: any, mockDB: any;
...
describe('/account/metrics_opt', () => {
  let logInstance: any, mockDB: any, mockCustoms: any;
