    ...
    <div className="w-full flex flex-col gap-3 items-center bg-white rounded-xl border-2 border-grey-100 px-5 pt-5 pb-3">
      <ul
        className={classNames(
          'relative gap-2 w-full text-black',
      ...
      >
        {(valueIsArray ? value : [value]).map((item) => (
          <li
            key={item}
            className="px-3 py-[10px] flex items-center gap-3 rounded-lg text-black text-lg leading-tight bg-gradient-to-tr from-blue-600/10 to-purple-500/10"
          >
            <CodeIcon className="w-6 h-auto" aria-hidden />
            {item}
          </li>
        ))}
