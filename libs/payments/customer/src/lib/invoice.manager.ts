...
import { stripeInvoiceToInvoicePreviewDTO } from './util/stripeInvoiceToInvoicePreviewDTO';
import { getMinimumChargeAmountForCurrency } from './util/getMinimumChargeAmountForCurrency';
import {
  InvalidInvoiceError,
  InvoiceSubscriptionItemNotFoundError,
...
export class InvoiceManager {
  ...
  async previewUpcomingForUpgrade({
    ...
    const invoicePreview = stripeInvoiceToInvoicePreviewDTO(upcomingInvoice);
    ...
    const subscriptionItem = subscriptions.data
      ...
      ?.find((subscription) => subscription.plan.id === upgradeFromPrice?.id);

    const firstSubItem = requestObject.subscription_items?.at(0);
    if (!firstSubItem) throw new InvoiceSubscriptionItemNotFoundError();
