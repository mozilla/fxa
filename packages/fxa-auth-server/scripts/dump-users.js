...
program
  ...
  .option('-i, --input <filename>', 'Input filename from which to read input if not specified on the command line')
...
function getItems (type) {
  ...
  if (typeof program[type] === 'string') {
    ...
    input = fs.readFileSync(path.resolve(__dirname, program.input)).toString('utf8')
  }
