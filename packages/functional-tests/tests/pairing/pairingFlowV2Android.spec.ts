 ...
 * real custom Firefox (Marionette) running the FxA-owned v2 authority flow, and
 ...
 *
 *   Authority (Marionette, custom build)     Supplicant (Android / Fenix)
 *   ------------------------------------     ----------------------------
 ...
 *
 * WRITTEN AHEAD. This is gated behind ANDROID_PAIRING_V2_ENABLED and skips by
 * default. It cannot pass until Firefox for Android implements the v2 pairing
 * web-channel commands (pair_oauth_start), tracked under the mobile work
 * (FXA-13732). Until then the v2 supplicant flow only runs on desktop
 * (pairingFlowV2.spec.ts). The authority side here is identical to that spec.
 *
 * Prerequisites (in addition to the v2 desktop ones): a booted Android emulator
 * with a v2-capable Fenix debug build, `yarn adb-reverse`, and the
 * webchannel-localhost manifest patch (see docs/pairing/README.md). Set
 * ANDROID_PAIRING_V2_ENABLED=1 to run; FIREFOX_BINARY must point at the custom
 * desktop build for the authority.
 */
...
test.describe.serial('v2 Android pairing flow', () => {
  ...
  test.beforeEach(async ({}, testInfo) => {
    if (!process.env.ANDROID_PAIRING_V2_ENABLED) {
      testInfo.skip(
        true,
        'Set ANDROID_PAIRING_V2_ENABLED=1 (needs a v2-capable Fenix — FXA-13732)'
      );
    }
    if (!process.env.FIREFOX_BINARY) {
      testInfo.skip(
        true,
        'FIREFOX_BINARY must point at a v2-capable desktop build'
      );
    }
  ...
  test('authority mints v2 QR and Android supplicant completes pairing', async ({
    ...
    // Prepare the supplicant before minting the (short-lived) channel.
    ...
    await test.step('Android device is registered on the account', async () => {
      const device = await pollForMobileDevice(
        target.authClient,
