...
export const cachedSignIn = async (
  ...
  try {
    ...
    const { authenticationMethods } = await authClient.accountProfile(sessionToken);
...
export async function handleNavigation(navigationOptions: NavigationOptions) {
  ...
  if (!isFullyVerified) {
    ...
    if (
      ...
      navigationOptions.signinData.verificationMethod === VerificationMethods.EMAIL_OTP
    ) {
      await navigationOptions.authClient.sessionResendVerifyCode(navigationOptions.signinData.sessionToken);
    }
