...
// What Pair/Index and App read: the fragment as a string, from wherever it is.
describe('getPairingHash', () => {
  it('returns the live fragment while the URL still has one', () => {
    setHash('#connected-services');
    capturePairingChannelParams();

    expect(getPairingHash()).toBe('#connected-services');
  });

  it('falls back to the captured fragment once startup has stripped it', () => {
    setHash(MOCK_PAIRING_HASH);
    capturePairingChannelParams();

    expect(window.location.hash).toBe('');
    expect(getPairingHash()).toBe(MOCK_PAIRING_HASH);
  });

  it('is empty when there is neither', () => {
    capturePairingChannelParams();

    expect(getPairingHash()).toBe('');
  });
});
