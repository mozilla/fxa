...
import { authenticateWithPasskey } from './signin-passkey-auth';
...
describe('authenticateWithPasskey', () => {
  it('completes the ceremony with the server and reports success', async () => {
    ...
    const result = await authenticateWithPasskey(args);
  ...
  it.each([
    ...
    async (_label, service) => {
      ...
      await authenticateWithPasskey(args);
  ...
  describe('WebAuthn failures', () => {
    ...
    it('re-throws non-WebAuthn errors from getCredential', async () => {
      ...
      await expect(authenticateWithPasskey(args)).rejects.toBe(err);
      expect(spies.completePasskeyAuthentication).not.toHaveBeenCalled();
    });
  });
  ...
  describe('PRF', () => {
    ...
    it('zeroes the PRF output when the completion call throws', async () => {
      const { args, spies } = build({
        keysRequired: true,
        withWrapMaterial: true,
      });
      spies.beginPasskeyAuthentication.mockResolvedValue(OPTIONS_WITH_PRF);
      // The extracted output is a view over this buffer, so zeroing it here is
      // observable through the credential the ceremony was handed.
      const credential = credentialWithPrf();
      (getCredential as jest.Mock).mockResolvedValue(credential);
      const refused = new Error('server refused');
      spies.completePasskeyAuthentication.mockRejectedValue(refused);

      await expect(authenticateWithPasskey(args)).rejects.toBe(refused);

      expect(
        new Uint8Array(credential.clientExtensionResults.prf.results.first)
      ).toEqual(new Uint8Array(PRF_OUT.length));
    });
