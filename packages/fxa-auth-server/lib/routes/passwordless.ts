...
class PasswordlessHandler {
   ...
   * - The email matches the forcedEmailAddress config
   * - It exists but has no password set (verifierSetAt === 0)
   */
  private isPasswordlessEligible(account: any | null, email: string): boolean {
    if (!account) {
      return true; // New account
    }

    const forced =
      this.config.passwordlessOtp &&
      this.config.passwordlessOtp.forcedEmailAddresses;
  ...
  async sendCode(request: AuthRequest) {
    ...
    // Check eligibility
    if (!this.isPasswordlessEligible(account, email)) {
  ...
  async confirmCode(request: AuthRequest) {
    ...
    // Check eligibility
    if (account && !this.isPasswordlessEligible(account, email)) {
