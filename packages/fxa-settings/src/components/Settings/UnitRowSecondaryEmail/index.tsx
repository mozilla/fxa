  ...
  const ChangePrimaryOnMount = ({ email }: { email: string }) => {
    ...
    const makeEmailPrimary = useCallback(
      async (email: string) => {
        try {
          ...
          console.warn('mingyuan not caught by guard:', e);
          alertBar.error(
            l10n.getString(
    ...
    useEffect(() => {
      (async () => {
        ...
        console.warn(
          'mingyuan ChangePrimaryOnMount useEffect calling makeEmailPrimary'
        );
        called.current = true;
    ...
    useEffect(() => {
      return () => {
        console.warn('mingyuan ChangePrimaryOnMount unmounted');
      };
    }, []);
    return <></>;
  };
              ...
              {attemptChangePrimary && (
                <MfaGuard
                  ...
                  onDismissCallback={async () => {
                    console.warn('mingyuan MfaGuard dismissed');
                    setAttemptChangePrimary(false);
                  }}
                >
