...
/**
 * Fallback: find and kill any orphaned key_server.js processes that are not
 * tracked by PID files (e.g. from a crashed run that didn't write PID files).
 */
export function killOrphanedKeyServers(): void {
  try {
    const output = execSync('pgrep -f "bin/key_server.js"', {
      encoding: 'utf-8',
      timeout: 5000,
    }).trim();
    ...
    for (const pid of pids) {
      try {
        process.kill(pid, 'SIGTERM');
        console.log(`[Process Registry] Killed orphaned key_server.js (PID: ${pid})`);
      } catch {
        // Already dead
      }
    }
