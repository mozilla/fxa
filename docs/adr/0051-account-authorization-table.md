...
- **Spurious Sync rows.** Desktop always requests the Sync scope regardless of `service`, so non-Sync Desktop sign-ins record an `apps/oldsync` consent row the user never intended. This is harmless for the silent-exchange gate — Sync is never granted silently, so the extra row is never acted on — but it's a false positive for lapsed-Sync work and for service restoration ([FXA-14152]), which reads the table to re-enable previously-authorized services. To be bandaided by skipping the Sync consent write for non-Sync Desktop sign-ins ([FXA-14263]); expected to fully resolve once Desktop moves to refresh tokens.
- **Removed tokens leave gaps.** The backfill will run some time after the table reached production and will only cover refresh tokens still present then, so a user who authorized earlier and then lost the token (account reset, `/destroy`, or disconnect) has no row and can look like they never authorized. Acceptable — [FXA-14101] makes a removed row the intended state for a disconnected service anyway, and coverage improves as new logins write rows.
...
[FXA-14263]: https://mozilla-hub.atlassian.net/browse/FXA-14263
[FXA-14152]: https://mozilla-hub.atlassian.net/browse/FXA-14152
