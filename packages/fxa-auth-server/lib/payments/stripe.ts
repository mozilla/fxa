...
export class StripeHelper extends StripeHelperBase {
  private readonly plansAndProductsCacheTtlSeconds: number;
  private readonly stripeTaxRatesCacheTtlSeconds: number;
  ...
  constructor(log: Logger, config: ConfigType, statsd: StatsD) {
    ...
    const redis = config.subhub.plansCacheTtlSeconds
      ? Redis(
          {
            ...config.redis,
            ...config.redis.subhub,
    ...
    // Call base class
    super(config, firestore, paymentConfigManager, statsd, redis, log);
   ...
   */
  protected async validatePlan(plan: Stripe.Plan): Promise<boolean> {
  ...
  async addTaxIdToCustomer(customer: Stripe.Customer, currency?: string) {
    ...
    if (taxId) {
      console.log('Calling this.stripe.customers.update', customer.id, {
        invoice_settings: {
