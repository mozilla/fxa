...
const ERROR_CONTEXT: Partial<Record<string, string>> = {
  NotAllowedError:
    "The user dismissed the prompt — or the requested credential is not available on this device. It may have been registered on a different device, or deleted from the device's passkey manager.",
  SecurityError:
  ...
  UnknownError:
    'The browser returned a null credential — unexpected and likely a platform bug.',
};
...
const ERROR_REFERENCE: { name: string; how: string }[] = [
  {
    ...
    how: "Cancel or dismiss the browser passkey prompt. Also occurs when the requested credential ID was registered on a different device or has been deleted from the device's passkey manager.",
  },
  {
    name: 'SecurityError',
  ...
  },
  {
    ...
    how: 'Register a passkey, copy its credential ID into excludeCredentials, then try to register the same passkey again — the authenticator rejects the duplicate.',
  },
  {
...
// PRF shape varies by operation:
// create → { enabled: true } when authenticator supports PRF
// get    → { results: { first: "<base64url>" } } when PRF eval was requested
  ...
  return (
    <>
      <FormField
        ...
        description="Shown in the passkey manager as the account identifier. Sets user.name and user.displayName."
      >
        <input
          id="create-email"
          type="email"
      ...
      <FormField label="PRF extension">
        ...
        <p className="text-xs text-grey-500 mt-0.5 ml-6">
          Required per product spec (1.2). Enables passwordless Sync sign-in via
          a passkey-derived key.
        </p>
      </FormField>
    </>
  );
  ...
  return (
    <>
      ...
      <FormField
        label="Credential ID"
        htmlFor="get-cred-id"
        description="Leave empty for discoverable credential flow — the browser shows all passkeys for this domain. Enter an ID to target a specific passkey."
      >
  ...
  return (
    <div className="min-h-screen bg-grey-10 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        >
          {editMode === 'form' ? (
            <CreationFormView form={createForm} onChange={setCreateForm} />
          ) : (
            <>
              <JsonTextarea
