...
const expectedBundle = 'eyJlbmMiOiJBMjU2R0NNIiwiYWxnIjoiZGlyIiwia2lkIjoiNmFhMjQ4OTMxNzA0ODg2ZjU0YWM2NGI4MWIxMTFiYzAifQ..' +
...
describe('lib/crypto/recovery-keys', () => {
  ...
  describe('generateRecoveryKey', () => {
    ...
    it('should prepend version', () => {
      return RecoveryKeys.generateRecoveryKey(1000)
        .then((key) => assert.equal(key.charAt(0), RecoveryKeys.getCurrentRecoveryKeyVersion()));
    });
  ...
  describe('bundleRecoveryData', () => {
    ...
    it('should create recovery bundle', () => {
      return RecoveryKeys.bundleRecoveryData(recoveryJwk, recoveryData, encryptionOptions)
        .then((bundle) => {
  ...
  describe('unbundleRecoveryData', () => {
    ...
    it('should unbundle recovery data', () => {
      return RecoveryKeys.unbundleRecoveryData(recoveryJwk, recoveryBundle)
        .then((bundle) => {
          assert.equal(bundle.kB, recoveryData.kB, 'decrypted bundle contains kB');
        });
    });
    ...
    it('should fail to unbundle with incorrect recovery key', () => {
      ...
      return RecoveryKeys.getRecoveryJwk(uid, recoveryKey)
        ...
        .then(assert.fail, (err) => {
          assert.equal(err.message, 'Failed to unbundle recovery data', 'correct error message');
        });
    });
  });
