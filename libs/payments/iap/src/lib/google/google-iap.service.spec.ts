...
describe('GoogleIapService', () => {
  ...
  describe('getActiveSubscriptions', () => {
    ...
    const inactiveSubscription =
      PlayStoreSubscriptionPurchase.fromFirestoreObject(firestoreRecord);
    inactiveSubscription.isEntitlementActive = jest.fn().mockReturnValue(false);

    it('returns active Google Play subscription purchases', async () => {
      jest
        .spyOn(manager, 'getForUser')
    ...
    it('returns [] if no active Play subscriptions are found', async () => {
      jest
        .spyOn(manager, 'getForUser')
        .mockResolvedValue([inactiveSubscription, inactiveSubscription]);
