...
test('dataflow', () => {
  ...
  test('messageHandler', () => {
    test('messages are acked, logged, and processed', () => {
      ...
      const messageData = JSON.stringify({ metadata: [{ key: 'customs_category', value: 'bar' }]})
      const message = {
        ack: sinon.spy(),
        id: 'message1',
      ...
      assert.isTrue(log.info.calledOnceWith({
        op: 'dataflow.message',
        count: 0,
        id: 'message1',
        data: messageData,
  ...
  test('processMessage', () => {
    test('parse error logs an error', () => {
      ...
      assert.isTrue(log.error.calledOnceWith({
  ...
  test('parseMessage', () => {
    ...
    test('invalid data throws an error (must be a Buffer)', () => {
      assert.throws(() => parseMessage({ data: 1 }), 'message data is not a Buffer')
    })

    test('invalid JSON throw an error', () => {
      assert.throws(() => parseMessage({ data: Buffer.from('{')}), 'Unexpected end of JSON input')
    })

    test('missing metadata throws an error', () => {
      assert.throws(() => parseMessage({ data: Buffer.from(JSON.stringify({}))}))
    })

    test('invalid metadata throws an error', () => {
      assert.throws(() => parseMessage({ data: Buffer.from(JSON.stringify({ metadata: 1 }))}))
    })
    ...
    test('valid message returns metadata array converted to an object', () => {
      assert.deepEqual(parseMessage({ data: Buffer.from(JSON.stringify({
        metadata: [{
          'key': 'foo',
          'value': 'bar'
        }, {
  ...
  test('checkForUnexpectedBlock', () => {
    ...
    const { checkForUnexpectedBlock } = dataflow(config, log, (...args) => fetchSpy(...args))
    ...
    test('rl_login_failure_sourceaddress_accountid', () => {
      test('no ipEmailRecord logs an unexpected block', async () => {
        ...
        assert.isTrue(fetchSpy.calledOnceWith({
          email: 'foo',
          ip: 'bar'
        }))
