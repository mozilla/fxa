...
describe('/session/reauth', () => {
  ...
  it('emits the correct series of calls', () => {
    ...
    return runTest(route, request).then((res: any) => {
      ...
      const checkCustomsArgs =
        signinUtils.checkCustomsAndLoadAccount.mock.calls[0];
      expect(checkCustomsArgs.length).toBe(3);
      ...
      expect(checkEmailArgs.length).toBe(3);
      expect(checkEmailArgs[0].uid).toBe(TEST_UID);
      expect(checkEmailArgs[1]).toBe(TEST_EMAIL);
      expect(checkEmailArgs[2]).toBeUndefined();

      expect(signinUtils.checkPassword).toHaveBeenCalledTimes(1);
      const checkPwArgs = signinUtils.checkPassword.mock.calls[0];
      expect(checkPwArgs.length).toBe(3);
      expect(checkPwArgs[0].uid).toBe(TEST_UID);
      expect(checkPwArgs[1].authPW.toString('hex')).toBe(TEST_AUTHPW);
      expect(checkPwArgs[2].app.clientAddress).toBe(knownIpLocation.ip);
      ...
      const sendNotifArgs = signinUtils.sendSigninNotifications.mock.calls[0];
      expect(sendNotifArgs.length).toBe(4);
      expect(sendNotifArgs[0]).toBe(request);
      expect(sendNotifArgs[1].uid).toBe(TEST_UID);
      expect(sendNotifArgs[2]).toBe(request.auth.credentials);
      expect(sendNotifArgs[3]).toBeUndefined();

      expect(signinUtils.createKeyFetchToken).toHaveBeenCalledTimes(1);
      const createKFTArgs = signinUtils.createKeyFetchToken.mock.calls[0];
      expect(createKFTArgs.length).toBe(4);
      expect(createKFTArgs[0]).toBe(request);
      expect(createKFTArgs[1].uid).toBe(TEST_UID);
      expect(createKFTArgs[2].authPW.toString('hex')).toBe(TEST_AUTHPW);
      expect(createKFTArgs[3]).toBe(request.auth.credentials);

      expect(signinUtils.getSessionVerificationStatus).toHaveBeenCalledTimes(1);
      expect(signinUtils.getSessionVerificationStatus).toHaveBeenNthCalledWith(
        1,
        request.auth.credentials,
        undefined
      );
...
describe('/session/verify/verify_push', () => {
  ...
  it('should verify push notification login request', async () => {
    ...
    expect(db.account).toHaveBeenCalledTimes(1);
    expect(db.account).toHaveBeenNthCalledWith(1, 'foo');
    expect(db.verifyTokens).toHaveBeenCalledTimes(1);
    expect(db.verifyTokens).toHaveBeenNthCalledWith(
      1,
      'sometoken',
      expect.anything()
    );
