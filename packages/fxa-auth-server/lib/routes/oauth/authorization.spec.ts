...
// The wiring, as distinct from the `isPairingAuthorization` unit tests above:
// that the /oauth/authorization handler actually reaches the Glean call, and
// that it stays silent for an ordinary sign-in by the same mobile client.
describe('/oauth/authorization pairing success event', () => {
  ...
  async function runHandler(opts: { clientId: string; userAgent: string }) {
    await jest.isolateModulesAsync(async () => {
      jest.doMock('../../oauth/assertion', () =>
        jest.fn(async () => ({ uid: UID_HEX }))
      );
      jest.doMock('../../oauth/grant', () => ({
        validateRequestedGrant: jest.fn(async (_claims, _client, payload) => ({
          clientId: Buffer.from(opts.clientId, 'hex'),
          userId: Buffer.from(UID_HEX, 'hex'),
          scope: payload.scope,
          offline: false,
        })),
      ...
      // The real one signs a JWT from config.oauth.secretKey, which baseConfig
      ...
      const routes = require('./authorization')({
        ...
        oauthDB: {
          getClient: jest.fn(async () => ({
            id: Buffer.from(opts.clientId, 'hex'),
            name: 'Firefox',
            trusted: true,
            publicClient: false,
            redirectUri: REDIRECT_URI,
          })),
          generateCode: jest.fn(async () => Buffer.alloc(32, 1)),
          recordSignInConsents: jest.fn(async () => {}),
        },
        config: baseConfig,
      });
      ...
      await routes[2].handler({
        headers: {},
        app: {
          ua: parseToScalars(opts.userAgent),
          geo: { location: {} },
  ...
  beforeEach(() => {
    mockGlean.pairing.success.mockClear();
  });

  it('records the event for a desktop Firefox authority', async () => {
    await runHandler({
      clientId: OAuthNativeClients.Fenix,
      userAgent: DESKTOP_FIREFOX_UA,
    });

    expect(mockGlean.pairing.success).toHaveBeenCalledTimes(1);
  });

  it('stays silent when the mobile client signs in for itself', async () => {
    await runHandler({
      clientId: OAuthNativeClients.Fenix,
      userAgent: ANDROID_FIREFOX_UA,
    });

    expect(mockGlean.pairing.success).not.toHaveBeenCalled();
  });
});
