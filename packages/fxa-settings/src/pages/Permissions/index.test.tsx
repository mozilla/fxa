...
it('shows the label alone when the account has no value for a scope', () => {
  renderPermissions([{ scope: 'profile:display_name' }]);
  const row = screen.getByTestId('permissions-row-profile-display_name');
  expect(row).toHaveTextContent('Display name');
...
it('shows an error banner when one is given', () => {
  renderPermissions(undefined, 'Something went wrong');
