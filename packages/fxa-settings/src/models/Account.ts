...
/** Response shape from auth-client getRecoveryKey */
interface RecoveryKeyBundleResponse {
  recoveryData: string;
}
...
/** Response shape for auth-client security events */
...
/** Response shape for auth-client attached clients */
export interface RawAttachedClient {
  clientId: string;
  ...
  location?: { city?: string | null; country?: string | null; state?: string | null; stateCode?: string | null };
  os: string | null;
...
export class Account implements AccountData {
  ...
  async refresh(
    ...
    await this.withLoadingStatus(
      (async () => {
        switch (field) {
          case 'clients':
            const clients = await this.authClient.attachedClients(token) as RawAttachedClient[];
            updateExtendedAccountState({
          ...
          case 'emails':
            ...
            break;
          case 'securityEvents':
            const events = await this.authClient.securityEvents(token) as RawSecurityEvent[];
            updateExtendedAccountState({
          ...
          default:
            ...
            if (clientsData.status === 'fulfilled') {
              updates.attachedClients = (clientsData.value as RawAttachedClient[]).map(mapAttachedClient);
            }
  ...
  async getRecoveryKeyBundle(
    ...
    try {
      ...
      return { recoveryData: (recoveryData as RecoveryKeyBundleResponse).recoveryData, recoveryKeyId };
    } catch (err: unknown) {
  ...
  async resetPasswordWithRecoveryKey(opts: {
    ...
    // in notifyClientOfSignin using storeAccountData, which properly sets both the account
    // and currentAccountUid. Don't do partial/broken storage here.
    return this.authClient.resetPasswordWithRecoveryKey(
