...
describe('hasSeenPermissions', () => {
  ...
  it('is true when every scope was seen', () => {
    mockGetAccountByUid.mockReturnValue({
      uid: UID,
      permissions: {
        [CLIENT_ID]: { 'profile:email': true, 'profile:display_name': true },
      },
    });
    expect(
  ...
  it('is false when one scope is new', () => {
    mockGetAccountByUid.mockReturnValue({
      uid: UID,
      permissions: { [CLIENT_ID]: { 'profile:email': true } },
    });
    expect(
      hasSeenPermissions(UID, CLIENT_ID, [
        'profile:email',
  ...
  it('is scoped per client', () => {
    mockGetAccountByUid.mockReturnValue({
      uid: UID,
      permissions: { dcdb5ae7add825d2: { 'profile:email': true } },
    });
    expect(hasSeenPermissions(UID, CLIENT_ID, ['profile:email'])).toBe(false);
  });

  it('is true for a scope the user declined, which the screen still showed', () => {
    mockGetAccountByUid.mockReturnValue({
      uid: UID,
      permissions: { [CLIENT_ID]: { 'profile:email': false } },
    });
    expect(hasSeenPermissions(UID, CLIENT_ID, ['profile:email'])).toBe(true);
  });

  it('reads the legacy grantedPermissions array', () => {
    mockGetAccountByUid.mockReturnValue({
      uid: UID,
...
describe('recordSeenPermissions', () => {
  it('adds the new scopes to what was already seen', () => {
    mockGetAccountByUid.mockReturnValue({
      uid: UID,
      permissions: { [CLIENT_ID]: { 'profile:email': true } },
    });
  ...
  it('writes permissions rather than the legacy field Backbone unsets', () => {
    mockGetAccountByUid.mockReturnValue({
      uid: UID,
      grantedPermissions: { [CLIENT_ID]: ['profile:email'] },
    });
    recordSeenPermissions(UID, CLIENT_ID, ['profile:display_name']);
    expect(mockPersistAccount).toHaveBeenCalledWith({
      uid: UID,
      permissions: { [CLIENT_ID]: { 'profile:display_name': true } },
    });
  });
  ...
  it('leaves other clients untouched', () => {
    ...
    expect(mockPersistAccount).toHaveBeenCalledWith({
      uid: UID,
      permissions: {
        dcdb5ae7add825d2: { 'profile:email': true },
        [CLIENT_ID]: { 'profile:email': true },
      },
    });
  });
...
describe('integrationNeedsPermissions', () => {
  ...
  it('is true for an untrusted web client the user has not seen', () => {
    expect(integrationNeedsPermissions(webIntegration, UID)).toBe(true);
  });

  it('is false for a native client', () => {
    expect(
      integrationNeedsPermissions(
        { ...webIntegration, type: IntegrationType.OAuthNative },
        UID
      )
    ).toBe(false);
  });
