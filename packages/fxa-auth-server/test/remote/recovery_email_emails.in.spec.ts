...
import type { EmailData } from '../support/helpers/mailbox';
import crypto from 'crypto';

// The raw email object returned by the mailbox helper includes a top-level
// `cc` array that the EmailData interface does not model; narrow to it for the
// CC assertions below.
type EmailDataWithCc = EmailData & { cc: { address: string }[] };
  ...
  ({ version, tag }) => {
    ...
    describe('should receive emails on verified secondary emails', () => {
      ...
      it('receives sign-in confirmation email', async () => {
        ...
        const emailCode = emailData['headers']['x-verify-code'];
        expect(emailCode).toBeTruthy();
        expect((emailData as EmailDataWithCc).cc.length).toBe(1);
        expect((emailData as EmailDataWithCc).cc[0].address).toBe(secondEmail);
        ...
        expect((emailData2 as EmailDataWithCc).cc.length).toBe(1);
        expect((emailData2 as EmailDataWithCc).cc[0].address).toBe(secondEmail);
      });
...
describe.each(testVersions)(
  '#integration$tag - remote emails new device signin',
  ({ version, tag }) => {
    const testOptions = { version };
    let skipServer: TestServerInstance;

    beforeAll(async () => {
      // Need a server with skipForNewAccounts enabled so new accounts
      // get verified sessions without signin confirmation
      await server.stop();
      skipServer = await createTestServer({
        configOverrides: {
          securityHistory: { ipProfiling: {} },
    ...
    afterAll(async () => {
      await skipServer.stop();
      // Restore original server for any subsequent test suites
      server = await createTestServer({
        configOverrides: {
          securityHistory: { ipProfiling: {} },
          signinConfirmation: { skipForNewAccounts: { enabled: false } },
    ...
    it('receives new device sign-in email', async () => {
      const email = skipServer.uniqueEmail();
      const secondEmail = skipServer.uniqueEmail();
      const thirdEmail = skipServer.uniqueEmail();

      const client = await Client.createAndVerify(
        skipServer.publicUrl,
        email,
        password,
        skipServer.mailbox,
        testOptions
      ...
      const code = await skipServer.mailbox.waitForCode(secondEmail);
      await client.verifySecondaryEmailWithCode(clientJwt, code, secondEmail);

      // Clear add secondary email notification
      await skipServer.mailbox.waitForEmail(email);

      // Create unverified email (triggers verification email but won't be verified)
      await client.createEmail(clientJwt, thirdEmail);

      // Login again
      await client.login({ keys: true });
      const emailData = await skipServer.mailbox.waitForEmail(email);
