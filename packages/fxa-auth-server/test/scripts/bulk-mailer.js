...
const account1Mock = createAccount('user1@test.com', 'f9916686c226415abd06ae550f073cec', 'en')
const account2Mock = createAccount('user2@test.com', 'f9916686c226415abd06ae550f073ced', 'es')
...
describe('scripts/bulk-mailer', function () {
  ...
  before(() => {
    ...
    return TestServer.start(config)
      ...
      .then(() => {
        return cp.execAsync(`node scripts/dump-users --uids ${account1Mock.uid},${account2Mock.uid} > ${USER_DUMP_PATH}`, { cwd })
      })
  })
  ...
  it('fails if --input missing', () => {
    return cp.execAsync('node scripts/bulk-mailer --method sendVerifyCode', { cwd })
  ...
  it('fails if --input file missing', () => {
    return cp.execAsync('node scripts/bulk-mailer --input does_not_exist --method sendVerifyCode', { cwd })
      .then(() => assert(false, 'script should have failed'), (err) => {
        assert.include(err.message, 'Command failed')
      })
  })
  ...
  it('fails if --method missing', () => {
    return cp.execAsync('node scripts/bulk-mailer --input ${USER_DUMP_PATH}', { cwd })
      .then(() => assert(false, 'script should have failed'), (err) => {
        assert.include(err.message, 'Command failed')
      })
  })
  ...
  it('fails if --method is invalid', () => {
    return cp.execAsync('node scripts/bulk-mailer --input ${USER_DUMP_PATH} --method doesNotExist', { cwd })
      .then(() => assert(false, 'script should have failed'), (err) => {
        assert.include(err.message, 'Command failed')
      })
  })
  ...
  it('succeeds with valid input file and method, writing files to disk', () => {
    return cp.execAsync(`node scripts/bulk-mailer --input ${USER_DUMP_PATH} --method sendVerifyCode --write ${OUTPUT_DIRECTORY}`, { cwd })
      .then(() => {
        assert.isTrue(fs.existsSync(path.join(OUTPUT_DIRECTORY, 'user1@test.com.headers')))
        assert.isTrue(fs.existsSync(path.join(OUTPUT_DIRECTORY, 'user1@test.com.html')))
        assert.isTrue(fs.existsSync(path.join(OUTPUT_DIRECTORY, 'user1@test.com.txt')))

        // emails are in english
        const test1Html = fs.readFileSync(path.join(OUTPUT_DIRECTORY, 'user1@test.com.html')).toString()
        assert.include(test1Html, 'This is an automated email')
        const test1Text = fs.readFileSync(path.join(OUTPUT_DIRECTORY, 'user1@test.com.txt')).toString()
        ...
        const test2Html = fs.readFileSync(path.join(OUTPUT_DIRECTORY, 'user2@test.com.html')).toString()
        assert.include(test2Html, 'Este es un correo automatizado')
        const test2Text = fs.readFileSync(path.join(OUTPUT_DIRECTORY, 'user2@test.com.txt')).toString()
  ...
  it('succeeds with valid input file and method, writing emails to stdout', () => {
    return cp.execAsync(`node scripts/bulk-mailer --input ${USER_DUMP_PATH} --method sendVerifyCode`, { cwd })
      .then(result => {
  ...
  it('succeeds with valid input file and method, sends', () => {
    return cp.execAsync(`node scripts/bulk-mailer --input ${USER_DUMP_PATH} --method sendVerifyCode --send`, { cwd })
  })
})
