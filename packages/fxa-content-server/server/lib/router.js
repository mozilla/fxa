...
module.exports = function (subdir, config) {
  const path = require('path');
  const i18n = require('./i18n')(config.get('i18n'));
  const routes = require(path.join(__dirname, 'routes', subdir))(config, i18n);

  routes.push(
    require('./routes/shared/get-lbheartbeat')(),
    require('./routes/shared/post-metrics')(),
    require('./routes/shared/post-metrics-errors')(),
    require('./routes/shared/get-500')(config)
  );

  if (config.get('csp.enabled')) {
    routes.push(require('./routes/shared/post-csp')({
  ...
  if (config.get('env') === 'development') {
    routes.push(require('./routes/shared/get-502')(config));
    routes.push(require('./routes/shared/get-503')(config));
    // Add a route in dev mode to test 500 errors
    routes.push(require('./routes/shared/get-boom')());
  }
