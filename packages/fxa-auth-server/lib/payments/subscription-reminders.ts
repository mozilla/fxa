...
export class SubscriptionReminders {
  ...
  /**
   * Determine if a discount is ending by comparing subscription discount with invoice preview.
   */
  private hasDiscountEnding(
    ...
    // Check if the invoice preview shows no discount or a different discount amount
    const invoiceDiscount = invoicePreview.discount;

    // If subscription has a discount but invoice doesn't, discount is ending
    if (!invoiceDiscount) {
      return true;
    }

    // If the discount amounts differ significantly, discount is ending
    // (Note: invoice preview may not include the discount if it's ending)
    return false;
  }
