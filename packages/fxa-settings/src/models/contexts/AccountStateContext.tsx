...
/**
 * Convert unified storage data to AccountState format
 */
function unifiedToAccountState(data: UnifiedAccountData | null, initialState?: Partial<AccountState>): AccountState {
  if (!data) {
    return {
      ...defaultAccountState,
      ...(initialState || {}),
...
export function AccountStateProvider({
  ...
  // Get current account data from unified storage
  const accountState = useMemo<AccountState>(() => {
    // accountsData triggers re-computation when localStorage changes
    void accountsData;
    // getAccountData handles migration and returns unified data
  ...
  const setAccountDataCallback = useCallback((data: Partial<AccountState>) => {
    ...
    // Convert Set to array for storage, and Error to serializable format
    // Also handle null -> undefined conversion for fields that differ between AccountState and UnifiedAccountData
