...
describe('PayPalHelper', () => {
  ...
  describe('getCheckoutToken', () => {
    ...
    it('throws a validation error with invalid options', async () => {
      try {
        await paypalHelper.getCheckoutToken({});
        assert.fail('Invalid options should throw an error.');
      } catch (err) {
        assert.ok(err.isJoi);
        assert.exists(err.details);
      }
    });
    ...
    it('calls setExpressCheckout with passed options', async () => {
      ...
      assert.ok(
        paypalHelper.client.setExpressCheckout.calledOnceWith(validOptions)
      );
    });
