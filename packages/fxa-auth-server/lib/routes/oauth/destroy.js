...
module.exports = ({ log, oauthDB }) => {
  async function destroyHandler(req) {
    ...
    // Settings and device disconnect paths: Firefox Desktop destroys its sync
    // refresh token here right after sign-in, so treating this as a withdrawal
    // would deauthorize the authorization the user just granted. Firefox Mobile does
    // not do this.
  ...
  return [
    {
      ...
      path: '/destroy',
      config: {
        ...OAUTH_SERVER_DOCS.DESTROY_POST,
        cors: { origin: 'ignore' },
        validate: {
