...
class EventsServerEventLogger {
  ...
  /**
   * Record and submit a passkey_wrap_create_complete event:
   * A passkey key-wrap envelope was stored for the first time. This is the last step of enrolling a passkey for passwordless Sync; a passkey that never reaches it cannot unseal the account's Sync keys. Records only the writes that stored a new envelope, so a client re-sending one it already stored is not counted.
   * Event is logged using internal mozlog logger.
   *
   * @param {string} user_agent - The user agent.
   * @param {string} ip_address - The IP address. Will be used to decode Geo
   ...
   * @param {string} utm_term - This metric is similar to the `utm.source`; it is used in the Firefox browser.  For example, if the user started from about:welcome, then the value could be 'aboutwelcome-default-screen'.  The value has a max length of 128 characters with the alphanumeric characters, _ (underscore), forward slash (/), . (period), % (percentage sign), and - (hyphen) in the allowed set of characters..
   */
  recordPasskeyWrapCreateComplete({
    user_agent,
    ip_address,
    account_user_id,
    account_user_id_sha256,
  ...
  /**
   * Record and submit a passkey_wrap_create_failure event:
   * Storing a passkey key-wrap envelope failed. A `conflict` reason means the client derived a different envelope for a credential that already has one, which points at a client crypto fault rather than a user error.
   * Event is logged using internal mozlog logger.
   *
   * @param {string} user_agent - The user agent.
   * @param {string} ip_address - The IP address. Will be used to decode Geo
   ...
   * @param {string} utm_source - The source from where the user started.  For example, if the user clicked on a link on the Mozilla accounts web site, this value could be 'fx-website'.  The value has a max length of 128 characters with the alphanumeric characters, _ (underscore), forward slash (/), . (period), % (percentage sign), and - (hyphen) in the allowed set of characters..
   * @param {string} utm_term - This metric is similar to the `utm.source`; it is used in the Firefox browser.  For example, if the user started from about:welcome, then the value could be 'aboutwelcome-default-screen'.  The value has a max length of 128 characters with the alphanumeric characters, _ (underscore), forward slash (/), . (period), % (percentage sign), and - (hyphen) in the allowed set of characters..
   * @param {string} reason - Why the write was refused. One of `not_bound` (the MFA token was minted for a different credential), `conflict` (a different envelope is already stored), `credential_not_found`, or `unexpected`..
   */
  recordPasskeyWrapCreateFailure({
    user_agent,
    ip_address,
    account_user_id,
    account_user_id_sha256,
  ...
  /**
   * Record and submit a passkey_wrap_get_complete event:
   * A passkey key-wrap envelope was served, letting the client unseal the account's Sync keys. Records only the attempts that succeeded; the ones that did not are counted by passkey.wrap_get_failure.
   * Event is logged using internal mozlog logger.
   *
   * @param {string} user_agent - The user agent.
   * @param {string} ip_address - The IP address. Will be used to decode Geo
  ...
  /**
   * Record and submit a passkey_wrap_get_failure event:
   * A passkey key-wrap envelope was withheld. Every reason ends the same way for the user, in a silent fall back to signing in with a password, so the reason is what distinguishes them.
   * Event is logged using internal mozlog logger.
   *
   * @param {string} user_agent - The user agent.
   * @param {string} ip_address - The IP address. Will be used to decode Geo
   ...
   * @param {string} utm_source - The source from where the user started.  For example, if the user clicked on a link on the Mozilla accounts web site, this value could be 'fx-website'.  The value has a max length of 128 characters with the alphanumeric characters, _ (underscore), forward slash (/), . (period), % (percentage sign), and - (hyphen) in the allowed set of characters..
   * @param {string} utm_term - This metric is similar to the `utm.source`; it is used in the Firefox browser.  For example, if the user started from about:welcome, then the value could be 'aboutwelcome-default-screen'.  The value has a max length of 128 characters with the alphanumeric characters, _ (underscore), forward slash (/), . (period), % (percentage sign), and - (hyphen) in the allowed set of characters..
   * @param {string} reason - Why the envelope was withheld. One of `not_bound` (the MFA token was minted for a different credential), `stale` (the envelope seals a kB the account has since replaced, so the passkey needs re-enrolling), `wrap_not_found` (the passkey was never given an envelope), `credential_not_found`, or `unexpected`..
   */
  recordPasskeyWrapGetFailure({
    user_agent,
    ip_address,
    account_user_id,
    account_user_id_sha256,
