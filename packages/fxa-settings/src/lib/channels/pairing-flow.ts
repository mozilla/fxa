...
// Bounded so a long-lived page cannot grow the buffer without limit.
const PLOG_BUFFER_LIMIT = 200;

export function plog(...args: unknown[]): void {
  // eslint-disable-next-line no-console
  console.log('[pair2]', ...args);
  // Also buffered on `window`: the functional tests cannot see the console of
  // the Marionette-driven authority, so they read the trace back from here.
  try {
    const w = window as unknown as { __pair2Log?: string[] };
    const log = (w.__pair2Log ??= []);
    log.push(
      args.map((a) => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')
    );
    if (log.length > PLOG_BUFFER_LIMIT) {
      log.shift();
    }
  } catch {
