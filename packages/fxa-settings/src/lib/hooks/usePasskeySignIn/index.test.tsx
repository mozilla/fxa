...
import {
  SensitiveData,
  SensitiveDataClient,
} from '../../sensitive-data-client';
import {
  ensureCanLinkAcountOrRedirect,
  handleNavigation,
} from '../../../pages/Signin/utils';
import { unwrapPasskeyKb } from '../../passkeys/wrap/consumption';
...
describe('usePasskeySignIn passwordless Sync opt-in material', () => {
  ...
  it('clears material left by an earlier ceremony before starting a new one', async () => {
...
describe('usePasskeySignIn passwordless Sync consumption', () => {
    ...
    async (reason) => {
      ...
      expect(spies.navigateWithQuery).toHaveBeenCalledWith(
        '/signin_passkey_fallback',
        {
          state: {
            passkeySurface: 'emailfirst',
            passkeyFallback: { credentialId: MOCK_CREDENTIAL.id, reason },
          },
        }
      );
      expect(
  ...
  it('stashes opt-in material and falls back when no wrap is stored', async () => {
    (unwrapPasskeyKb as jest.Mock).mockResolvedValue({
      ok: false,
      reason: 'no_wrap',
    });
