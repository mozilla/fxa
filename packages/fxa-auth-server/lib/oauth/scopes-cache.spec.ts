...
describe('ScopeIdCache', () => {
  ...
  it('resolves a scope case-insensitively (scopes.scope is a CI unique key)', async () => {
    // DB IN(...) under CI collation matches "Profile" and returns the canonical
    // lower-cased row; the cache must map it back to the requested casing.
    resolver.mockResolvedValueOnce([{ scope: 'profile', id: 3 }]);

    const { resolved, missing } = await cache.resolve(['Profile']);

    expect(resolved.get('Profile')).toBe(3);
    expect(missing).toEqual([]);
  });

  it('serves a differently-cased scope from cache without re-querying', async () => {
    await cache.resolve(['profile']);
    const { resolved } = await cache.resolve(['PROFILE']);
