...
button-passkey-signin = 使用通行密钥登录
...
button-passkey-signin-loading = 正在安全登录…
...
cs-scope-firefox-vpn = { -brand-firefox } 的内置 VPN
...
product-promo-vpn-description = 探索更进一步的匿名浏览和保护方式。
...
passkey-delete-error = 删除您的通行密钥时出现问题，请过几分钟再试。
...
passkey-row-max-limit-disabled-reason = 您的通行密钥数量已达上限。
...
# Authenticator I/O failure (e.g., security key disconnected mid-ceremony)
passkey-authentication-error-not-readable = 我们无法访问身份验证器，请重试或使用其他登录方式。
# Catch-all for unexpected errors during authentication (TypeError, DataError, EncodingError, ConstraintError, OperationError, UnknownError)
passkey-authentication-error-unexpected = 出了点问题，请重试或选择其他登录方式。
...
passkey-authentication-error-not-found = 无法识别通行密钥，请使用其他登录方式。
...
pair-auth-allow-refuse-device-link = 若非本人操作，请<a>更改密码</a>
...
pair-failure-header-v2 = 设备配对失败
pair-failure-message-v2 = 无法完成设置，请通过邮箱地址登录。
pair-failure-try-again-link = 重试

## Pair index page

pair-sync-header = 与手机或平板电脑上的 { -brand-firefox } 同步
pair-cad-header-v2 = 连接其他设备
...
service-welcome-signin-success-banner = 登录成功！
# In this context, "VPN" is a VPN service built into the Firefox browser, and generally isn't localized differently than "VPN"
service-welcome-vpn-heading = 下一步: 开启 VPN
...
set-password-passwordless-info = 此密码用于加密您的同步数据，以确保其安全。
...
signin-passwordless-code-expired = 确认码已过期？
# { $seconds } - countdown timer showing seconds until user can request a new code
signin-passwordless-code-resend-countdown = { $seconds } 秒后可重新发送
...
signin-passwordless-totp-required = 您的账户已启用两步验证，请使用密码登录。
