...
// SIGNED cast prevents BIGINT UNSIGNED underflow when the new lastUsedAt
// is older than the stored value (out-of-order or backdated writes).
...
class MysqlStore extends MysqlOAuthShared {
  ...
  // SQL advances lastUsedAt only when the new value is more than throttleMs
  // past the existing one. Pass 0 for unthrottled writes (login).
