...
describe('db.deviceFromRefreshTokenId:', () => {
  ...
  it('should throw error and increment metrics when device is not found', async () => {
    const uid = 'test-uid';
    const refreshTokenId = 'test-refresh-token-id';
    const metrics = {
      increment: sinon.spy(),
    ...
    try {
      await db.deviceFromRefreshTokenId(uid, refreshTokenId);
      assert.fail('should have thrown');
    } catch (err) {
      assert.equal(err.errno, 110);
      assert.equal(err.message, 'Unknown device');
      assert.equal(Device.findByUidAndRefreshTokenId.callCount, 1);
      assert.equal(Device.findByUidAndRefreshTokenId.args[0][0], uid);
      assert.equal(
        Device.findByUidAndRefreshTokenId.args[0][1],
        refreshTokenId
      );
  ...
  it('should throw error when normalized device is null', async () => {
    const uid = 'test-uid';
    const refreshTokenId = 'test-refresh-token-id';
    const mockDevice = {
      id: 'device-id',
    ...
    Device.findByUidAndRefreshTokenId.resolves(mockDevice);
    features.isLastAccessTimeEnabledForUser.returns(false);
    mergeDevicesAndSessionTokens.returns([null]);
  ...
  it('should throw error when normalized device is undefined', async () => {
    const uid = 'test-uid';
    const refreshTokenId = 'test-refresh-token-id';
    const mockDevice = {
      id: 'device-id',
    ...
    Device.findByUidAndRefreshTokenId.resolves(mockDevice);
    features.isLastAccessTimeEnabledForUser.returns(false);
    mergeDevicesAndSessionTokens.returns([undefined]);
