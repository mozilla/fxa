...
firefox-promo-banner-mobile-heading = 不論您身在何處，都能下載 { -brand-firefox }
firefox-promo-banner-mobile-description = 在您的所有裝置間同步分頁、書籤、密碼。所有內容都經安全加密。
firefox-promo-banner-mobile-cta = 連結裝置
firefox-promo-banner-switch-heading = 快速切換、容易適應。
firefox-promo-banner-switch-description = 改用 { -brand-firefox } 時，還是可以將書籤、密碼、瀏覽紀錄等資料帶過來，上網不漏拍。
firefox-promo-banner-switch-cta = 改用 { -brand-firefox }
...
page-passkey-add-follow-prompts = 請依照裝置上的提示操作。
...
recent-activity-account-mfa-otp-sent = 請求授權帳號變更
...
product-promo-vpn-description = 探索更多一層的匿名上網與保護機制。
...
passkey-registration-canceled-v2 = Passkey 設定逾時或取消
passkey-registration-canceled-link = 更多資訊
passkey-registration-error-not-supported-v2 = 您的瀏覽器或裝置不支援 Passkey。
passkey-registration-error-not-supported-link = 更多資訊
passkey-registration-error-could-not-complete = 無法完成 Passkey 設定，請改用另一種方式或裝置。
passkey-registration-error-could-not-complete-link = 更多資訊
...
passkey-authentication-error-not-found = 無法識別的 Passkey。請改用其他登入方式。
...
signin-recovery-error = 某些東西不對勁，請重新登入。
