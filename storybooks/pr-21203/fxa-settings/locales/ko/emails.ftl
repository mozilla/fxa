...
automated-email-sign-in =
    자동으로 발송된 이메일입니다. 이 작업을 승인하지 않았다면 <a data-l10n-name="securitySettingsLink">계정 보안 설정을 확인</a>하세요.
    자세한 내용은 <a data-l10n-name="supportLink">{ -brand-mozilla } 지원</a>을 참조하세요.
automated-email-sign-in-plaintext = 이 작업을 승인하지 않았다면 계정 보안 설정을 확인하세요:
...
postAddPasskey-subject = 패스키 생성됨
postAddPasskey-preview = 이제 기기를 사용해 로그인할 수 있습니다.
postAddPasskey-title = 패스키를 생성했습니다.
postAddPasskey-description = 이제 이 계정으로 모든 { -product-mozilla-account } 서비스에 로그인할 수 있습니다.
postAddPasskey-sync-note = 주의하세요, 여전히 { -brand-firefox } 동기화 데이터에 접근하려면 비밀번호가 필요합니다.
# Links out to a support article about passkeys and { -brand-firefox } sync
postAddPasskey-learn-more = 더 알아보기
postAddPasskey-requested-from = 다음에서 요청함:
postAddPasskey-action = 계정 관리
...
postRemovePasskey-subject = 패스키 삭제됨
postRemovePasskey-preview = 패스키가 계정에서 삭제되었습니다.
postRemovePasskey-title = 패스키를 삭제했습니다.
postRemovePasskey-description = 로그인하려면 다른 방법을 이용해야 합니다.
postRemovePasskey-requested-from = 다음에서 요청함:
postRemovePasskey-action = 계정 관리
