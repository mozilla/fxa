...
test.describe('severity-1 #smoke', () => {
  test.describe('heuristic unverified session', () => {
    test.beforeEach(({ target }) => {
      test.skip(
        target.name !== 'local',
        'the forcedHeuristicEmailAddresses default is only deployed locally until the train reaches stage and prod'
      );
    });
    ...
    test('RP in servicesWithEmailVerification lands on signin_token_code and sends exactly one verifyLoginCode', async ({
      target,
      ...
      // Payments Next is in `servicesWithEmailVerification` locally. It is a
      // confidential client, so no PKCE params: the grant rejects them.
      ...
      // code is accepted. The grant is the assertion: Payments Next is not
      // running locally to land on.
