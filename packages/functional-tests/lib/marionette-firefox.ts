...
/**
 * Manages a real Firefox process with Marionette enabled for pairing tests.
 *
 * Launches Firefox with a temp profile containing FXA + Marionette prefs,
 * connects a MarionetteClient over TCP, and handles cleanup (session deletion,
 * process termination, temp profile removal) on close.
 *
 * Used as the "authority" browser in pairing E2E tests — the desktop Firefox
 * that generates the QR code and approves the supplicant device.
 */
export class MarionetteFirefox {
  ...
  static async launch(
    ...
    await waitForPortFree(marionettePort, 5000);
  ...
  async close(): Promise<void> {
    ...
    // Terminate Firefox: SIGTERM for graceful shutdown, escalate to SIGKILL after 5s
    if (this.process) {
    ...
    // Wait for the Marionette port to be fully released before returning,
    // so sequential tests reusing the same port don't hit EADDRINUSE.
    await waitForPortFree(this.marionettePort, 10000);
  }

  private get marionettePort(): number {
    return this.client['port'] ?? 2828;
  }
}
...
async function waitForPort(port: number, timeoutMs: number): Promise<void> {
  ...
  throw new Error(`Marionette port ${port} not available after ${timeoutMs}ms`);
}
...
/**
 * Kill any Firefox/Marionette process listening on the given port
 * (stale leftovers from failed tests). Only kills processes whose
 * command line contains "firefox" to avoid killing unrelated services.
 */
function killProcessOnPort(port: number): void {
  if (!Number.isFinite(port) || port < 1 || port > 65535) return;

  try {
    // Use -c flag to include the command name in output for safety check
    ...
    for (const pid of output.split('\n')) {
      ...
      // Verify this is actually a Firefox process before killing
      try {
        const cmdline = execSync(`ps -p ${parsedPid} -o comm=`, {
          encoding: 'utf-8',
        })
          .trim()
          .toLowerCase();
        if (!cmdline.includes('firefox')) {
          continue;
        }
      } catch {
        // ps failed — process may have exited, skip it
        continue;
      }
 ...
 * Polls with tryConnect; resolves when connection fails (port free).
 */
