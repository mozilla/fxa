...
/**
 * The pairing fragment, `#`-prefixed, from wherever it currently lives: the URL
 * when it is still there, otherwise the startup capture. Empty when neither.
 *
 * For readers that parse the fragment as a string — `parsePairingHash` in
 * Pair/Index and the v=2 check in App — which the startup strip would otherwise
 * leave looking at an empty hash on the very page the QR scan lands on.
 */
export function getPairingHash(): string {
  const live = window.location.hash ?? '';
  if (live.replace(/^#/, '')) {
    return live;
  }
  const captured = hashSnapshot ?? read(HASH_STORAGE_KEY);
