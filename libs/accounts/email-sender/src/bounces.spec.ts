...
/**
 * Basic helper to call an async function can capture then return the error.
 * @param callback
 * @returns
 */
const safeThrow = async (callback: Promise<any>) => {
  ...
  // AppError isn't typed strong enough to infer the "extra" param that gets applied to
  // the output.payload. This extends AppError to allow access to both the standard payload
  // properties (code, errno, error, message) AND the extra fields like bouncedAt.
  return thrownError as AppError & {
    output: { payload: AppError['output']['payload'] & Record<string, any> };
  };
};
...
describe('Bounces', () => {
  ...
  describe('check', () => {
    ...
    it('returns void if ignoreTemplates includes current template', async () => {
      const config: BouncesConfig = {
        ...defaultBouncesConfig,
  ...
  describe('checkBounces', () => {
    it('returns undefined if template is in ignoreTemplates', async () => {});
    ...
    it('throws emailBounceHard error when hard bounce over threshold', async () => {
      ...
      const thrownError = await safeThrow(
        bounces.check('test@example.com', 'verifyEmail')
      );
      expect(thrownError).toBeDefined();
      expect(thrownError.errno).toEqual(ERRNO.BOUNCE_HARD);
      expect(thrownError.output.payload.code).toEqual(400);
      expect(thrownError.output.payload.error).toEqual('Bad Request');
      expect(thrownError.output.payload.message).toEqual(
        'Email account hard bounced'
      );
      expect(thrownError.output.payload['bouncedAt']).toEqual(bouncedAt);
    });
    ...
    it('throws emailComplaint error when complaint bounce over threshold', async () => {
      ...
      expect(thrownError).toBeDefined();
      expect(thrownError.errno).toEqual(ERRNO.BOUNCE_COMPLAINT);
      expect(thrownError.output.payload.code).toEqual(400);
      expect(thrownError.output.payload.error).toEqual('Bad Request');
      expect(thrownError.output.payload.message).toEqual(
    ...
    it('throws emailBouncedSoft error when soft bounce over threshold', async () => {
      ...
      expect(thrownError).toBeDefined();
      expect(thrownError.errno).toEqual(ERRNO.BOUNCE_SOFT);
      expect(thrownError.output.payload.code).toEqual(400);
      expect(thrownError.output.payload.error).toEqual('Bad Request');
      expect(thrownError.output.payload.message).toEqual(
    ...
    it('sorts multiple bounces to descending order', async () => {
      ...
      expect(thrownError.output.payload['bouncedAt']).toEqual(latestBouncedAt);
    });
    ...
    it('handles multiple rules for the same bounce type', async () => {
      ...
      // this is interesting, even though the "latest" bouncedAt isn't the one
      // that trips the rules, it's what is returned if any rule is tripped.
      expect(thrownError.output.payload['bouncedAt']).toEqual(latestBouncedAt);
    });
