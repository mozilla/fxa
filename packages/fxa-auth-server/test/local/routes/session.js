...
describe('/session/reauth', () => {
  ...
  let log, config, customs, db, mailer, signinUtils, routes, route, request, SessionToken;
  ...
  beforeEach(() => {
    ...
    signinUtils = require('../../../lib/routes/utils/signin')(log, config, customs, db, mailer);
    SessionToken = require('../../../lib/tokens/index')(log, config).SessionToken;
  ...
  it('emits the correct series of calls', () => {
    ...
    signinUtils.checkCustomsAndLoadAccount = sinon.spy(() => P.props({ accountRecord: db.accountRecord(TEST_EMAIL) }));
    signinUtils.sendSigninNotifications = sinon.spy(() => P.resolve());
    signinUtils.createKeyFetchToken = sinon.spy(() => P.resolve( { data: 'KEYFETCHTOKEN' }));
    signinUtils.getSessionVerificationStatus = sinon.spy(() => ({ verified: true }));
    const testNow = Math.floor(Date.now() / 1000);
    return runTest(route, request).then((res) => {
      assert.equal(signinUtils.checkCustomsAndLoadAccount.callCount, 1, 'checkCustomsAndLoadAccount was called');
      let args = signinUtils.checkCustomsAndLoadAccount.args[0];
      assert.equal(args.length, 2, 'checkCustomsAndLoadAccount was called with correct number of arguments');
      assert.equal(args[0], request, 'checkCustomsAndLoadAccount was called with request as first argument');
      assert.equal(args[1], TEST_EMAIL, 'checkCustomsAndLoadAccount was called with email as second argument');

      assert.equal(db.accountRecord.callCount, 1, 'db.accountRecord was called');
      args = db.accountRecord.args[0];
      assert.equal(args.length, 1, 'db.accountRecord was called with correct number of arguments');
      assert.equal(args[0], TEST_EMAIL, 'db.accountRecord was called with email as first argument');

      assert.equal(signinUtils.checkEmailAddress.callCount, 1, 'checkEmaiLAddress was called');
      args = signinUtils.checkEmailAddress.args[0];
      assert.equal(args.length, 3, 'checkEmailAddress was called with correct number of arguments');
      assert.equal(args[0].uid, TEST_UID, 'checkEmailAddress was called with account record as first argument');
      assert.equal(args[1], TEST_EMAIL, 'checkEmaiLAddress was called with email as second argument');
      assert.equal(args[2], undefined, 'checkEmaiLAddress was called with undefined originalLoginEmail as third argument');

      assert.equal(signinUtils.checkPassword.callCount, 1, 'checkPassword was called');
      ...
      assert.equal(args.length, 1, 'db.updateSessionToken was called with correct number of arguments');
      assert.equal(args[0], request.auth.credentials, 'db.updateSessionToken was called with sessionToken as first argument');

      assert.equal(signinUtils.sendSigninNotifications.callCount, 1, 'sendSigninNotifications was called');
      args = signinUtils.sendSigninNotifications.args[0];
      assert.equal(args.length, 4, 'sendSigninNotifications was called with correct number of arguments');
      assert.equal(args[0], request, 'sendSigninNotifications was called with request as first argument');
      assert.equal(args[1].uid, TEST_UID, 'sendSigninNotifications was called with account record as second argument');
      assert.equal(args[2], request.auth.credentials, 'sendSigninNotifications was called with sessionToken as third argument');
      assert.equal(args[3], undefined, 'sendSigninNotifications was called with undefined verificationMethod as third argument');

      assert.equal(signinUtils.createKeyFetchToken.callCount, 1, 'createKeyFetchToken was called');
      args = signinUtils.createKeyFetchToken.args[0];
      assert.equal(args.length, 4, 'createKeyFetchToken was called with correct number of arguments');
      assert.equal(args[0], request, 'createKeyFetchToken was called with request as first argument');
      assert.equal(args[1].uid, TEST_UID, 'createKeyFetchToken was called with account record as second argument');
      assert.equal(args[2].authPW.toString('hex'), TEST_AUTHPW, 'createKeyFetchToken was called with Password object as third argument');
      assert.equal(args[3], request.auth.credentials, 'createKeyFetchToken was called with sessionToken as fourth argument');

      assert.equal(signinUtils.getSessionVerificationStatus.callCount, 1, 'getSessionVerificationStatus was called');
      args = signinUtils.getSessionVerificationStatus.args[0];
      assert.equal(args.length, 2, 'getSessionVerificationStatus was called with correct number of arguments');
      assert.equal(args[0], request.auth.credentials, 'getSessionVerificationStatus was called with sessionToken as first argument');
      assert.equal(args[1], undefined, 'getSessionVerificationStatus was called with undefined verificationMethod as first argument');

      assert.equal(Object.keys(res).length, 4, 'response object had correct number of keys');
      assert.equal(res.uid, TEST_UID, 'response object contained correct uid');
      assert.ok(res.authAt >= testNow, 'response object contained an updated authAt timestamp');
      assert.equal(res.keyFetchToken, 'KEYFETCHTOKEN', 'response object contained the keyFetchToken');
      assert.equal(res.verified, true, 'response object indicated correct verification status');
    });
  });
  ...
  it('correctly updates sessionToken details', () => {
    ...
    assert.ok(! request.auth.credentials.authAt, 'sessionToken starts off with no authAt');
    assert.ok(request.auth.credentials.lastAuthAt() < testNowSeconds , 'sessionToken starts off with low lastAuthAt');
    assert.ok(! request.auth.credentials.uaBrowser, 'sessionToken starts off with no uaBrowser');
    assert.ok(! request.auth.credentials.uaBrowserVersion, 'sessionToken starts off with no uaBrowserVersion');
    assert.ok(! request.auth.credentials.uaOS, 'sessionToken starts off with no uaOS');
    assert.ok(! request.auth.credentials.uaOSVersion, 'sessionToken starts off with no uaOSVersion');
    assert.ok(! request.auth.credentials.uaDeviceType, 'sessionToken starts off with no uaDeviceType');
    assert.ok(! request.auth.credentials.uaFormFactor, 'sessionToken starts off with no uaFormFactor');

    return runTest(route, request).then((res) => {
      assert.equal(db.updateSessionToken.callCount, 1, 'db.updateSessionToken was called');
      const sessionToken = db.updateSessionToken.args[0][0];
      assert.ok(sessionToken.authAt >= testNow, 'sessionToken has updated authAt timestamp');
      assert.ok(sessionToken.lastAuthAt() >= testNowSeconds , 'sessionToken has udpated lastAuthAt');
      assert.equal(sessionToken.uaBrowser, 'Firefox', 'sessionToken has updated uaBrowser');
      assert.equal(sessionToken.uaBrowserVersion, '50', 'sessionToken has updated uaBrowserVersion');
      assert.equal(sessionToken.uaOS, 'Android', 'sessionToken has updated uaOS');
      assert.equal(sessionToken.uaOSVersion, '6', 'sessionToken has updated uaOSVersion');
      assert.equal(sessionToken.uaDeviceType, 'mobile', 'sessionToken has updated uaDeviceType');
      assert.equal(sessionToken.uaFormFactor, 'trapezoid', 'sessionToken has updated uaFormFactor');
    });
  });
  ...
  it('leaves mustVerify=false when not requesting keys', () => {
    ...
    return runTest(route, request).then((res) => {
      assert.equal(db.updateSessionToken.callCount, 1, 'db.updateSessionToken was called');
      const sessionToken = db.updateSessionToken.args[0][0];
      assert.ok(! sessionToken.mustVerify, 'sessionToken still has mustVerify=false');
    });
  });

  it('does not return a keyFetchToken when not requesting keys', () => {
    signinUtils.checkPassword = sinon.spy(() => { return P.resolve(true); });
    signinUtils.createKeyFetchToken = sinon.spy(() => { assert.fail('should not be called'); });
    request.query.keys = false;

    return runTest(route, request).then((res) => {
      assert.equal(signinUtils.createKeyFetchToken.callCount, 0, 'createKeyFetchToken was not called');
      assert.ok(! res.keyFetchToken, 'response object did not contain a keyFetchToken');
    });
  });

  it('correctly rejects incorrect passwords', () => {
    signinUtils.checkPassword = sinon.spy(() => { return P.resolve(false); });

    return runTest(route, request).then(
      (res) => { assert.fail('request should have been rejected'); },
      (err) => {
        assert.equal(signinUtils.checkPassword.callCount, 1, 'checkPassword was called');
        assert.equal(err.errno, error.ERRNO.INCORRECT_PASSWORD, 'the errno was correct');
      }
    );
  });
  ...
  it('reflects the requested verificationMethod in the response body', () => {
    ...
    return runTest(route, request).then((res) => {
      assert.ok(res);
      assert.equal(res.verified, false, 'result reports the session as unverified');
