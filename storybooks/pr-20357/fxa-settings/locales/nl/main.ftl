...
link-sr-new-window = Opent in nieuw venster
...
button-passkey-signin-loading = Veilig aanmelden…
...
dark-mode-toggle-label = Thema wisselen
...
passkey-sub-row-created-date = Gemaakt: { $createdDate }
passkey-sub-row-last-used-date = Laatst gebruikt: { $lastUsedDate }
passkey-sub-row-sign-in-only = Alleen aanmelden. Kan niet worden gebruikt om te synchroniseren.
...
passkey-row-header = Wachtwoordsleutels
passkey-row-enabled = Ingeschakeld
...
passkey-row-info-link = Hoe dit uw account beschermt
...
set-password-passwordless-info = Dit wachtwoord versleutelt uw gesynchroniseerde gegevens en houdt ze veilig.
...
signin-recovery-error = Er is iets misgegaan. Meld u opnieuw aan.
...
signin-passkey-fallback-header = Aanmelding voltooien
...
signin-passwordless-code-subheading = Aanmelden vereist slechts één stap als u deze code gebruikt.
...
signin-passwordless-code-resend-link = Nieuwe code versturen per e-mail.
signin-passwordless-code-resend-error = Er is iets misgegaan. Er kon geen nieuwe code worden verzonden.
signin-passwordless-code-other-account-link = Een andere account gebruiken


signup-passwordless-code-subheading = Registratie vereist slechts één stap als u deze code gebruikt.
