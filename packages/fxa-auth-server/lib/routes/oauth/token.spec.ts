  ...
  describe('validateTokenExchangeGrant', () => {
    ...
    it('rejects unauthorized scopes', async () => {
      ...
      expect(mockStatsD.increment).toHaveBeenCalledWith(
        ...
        { service: 'legacy', outcome: 'rejected_legacy_allowlist' }
      );
    });
    ...
    it('rejects a multi-scope request that includes a silent-disallowed service', async () => {
      // Scope contains both relay (silent-allowed) and oldsync (silent-
      // disallowed). Must reject; first-match resolution would have leaked
      // oldsync via combinedScope.union.
      jest.resetModules();
      jest.doMock('../../oauth/assertion', () => async () => true);
      jest.doMock('../../oauth/client', () => ({
        authenticateClient: (_: any, params: any) => ({
      ...
      const request = {
        ...
        payload: {
          ...
          subject_token: REFRESH_TOKEN,
          subject_token_type: SUBJECT_TOKEN_TYPE_REFRESH,
          scope: `${OAUTH_SCOPE_RELAY} ${OAUTH_SCOPE_OLD_SYNC}`,
        },
        emitMetricsEvent: () => {},
      };
      await expect(routes[0].config.handler(request)).rejects.toMatchObject({
    ...
    it('rejects a non-Relay configured scope when no accountAuthorizations row exists', async () => {
      ...
      expect(mockStatsD.increment).toHaveBeenCalledWith(
        'oauth.token_exchange.resolution',
        { service: 'smartwindow', outcome: 'rejected_no_row' }
      );
    });
    ...
    it('allows a non-Relay configured scope when an accountAuthorizations row exists', async () => {
      ...
      expect(mockStatsD.increment).toHaveBeenCalledWith(
        'oauth.token_exchange.resolution',
        { service: 'smartwindow', outcome: 'granted' }
      );
    });
