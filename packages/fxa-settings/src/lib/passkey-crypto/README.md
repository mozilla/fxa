...
Two crypto layers, kept separate. Both bind the same credential binding —
length-framed `uid` and `credentialId`, built by the internal `encoding.ts` —
each under its own domain-separation label.
...
- Everything crosses the module boundary as `Uint8Array`, apart from the `CredentialContext` each function takes. No `CryptoKey` escapes a function.
- Callers pass `uid` and `credentialId`, never the binding bytes. Each layer frames them itself, so seal and open cannot disagree: assembling the framing at two call sites is how you get envelopes that nothing can open, and the failure is indistinguishable from a wrong PRF output.
...
- **Nothing generation-specific may enter either layer's binding.** An earlier draft bound `keysChangedAt` into the HPKE `aad`. It could never reach the `skR` wrap — rotation re-seals `kB` without the authenticator (tech spec §2.2.1, goal 4), so it has no `prfOut`, and a generation bound there would lock the credential out on every password reset. Confined to one layer it bought only a replay guard the row's own lifecycle already gives.
...
`golden-envelope.test.ts` decrypts a committed v1 envelope, and is the only test that fails when the format itself moves — everything else seals and opens with the same code, so it passes whatever the format is. Never regenerate that fixture to make a failure go away; that deletes the signal along with the failure.

`generate-v1-envelope-fixture.mjs` rebuilds it for the one case that is not a bug: a deliberate, reviewed format change. It restates the ciphersuite, framing, and labels rather than importing them — a generator that called `sealKb` would agree with the module by construction and pin nothing.
