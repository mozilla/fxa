...
const TESTS: [string, any, Record<string, any>?][] = [
  ...
  ['verifyLoginCodeEmail', new Map<string, Test | any>([
    ['subject', { test: 'equal', expected: 'Confirm Your Login' }],
    ['headers', new Map([
      ['X-SES-MESSAGE-TAGS', { test: 'equal', expected: 'messageType=fxa-verifyLoginCode, app=fxa, service=fxa-auth-server, ses:feedback-id-a=fxa-verifyLoginCode, cmsRp=00f00f-wibble' }],
      ['X-Signin-Verify-Code', { test: 'equal', expected: MESSAGE.code }],
      ['X-Template-Name', { test: 'equal', expected: 'verifyLoginCode' }],
      ['X-Template-Version', { test: 'equal', expected: TEMPLATE_VERSIONS.verifyLoginCode }],
    ])],
    ['html', [
      { test: 'include', expected: 'Are you signing in to use Product?' },
      { test: 'include', expected: 'Use the code below to verify your signin.' },
      { test: 'include', expected: decodeUrl(configHref('initiatePasswordChangeUrl', 'new-signin-verify-code', 'change-password', 'email')) },
      { test: 'include', expected: decodeUrl(configHref('privacyUrl', 'new-signin-verify-code', 'privacy')) },
      { test: 'include', expected: decodeUrl(configHref('supportUrl', 'new-signin-verify-code', 'support')) },
      { test: 'include', expected: MESSAGE.code },
  ...
  ['postSigninRecoveryPhoneEmail', new Map<string, Test | any>([
    ['subject', { test: 'equal', expected: 'Recovery phone used to sign in' }],
    ['headers', new Map([
      ['X-SES-MESSAGE-TAGS', { test: 'equal', expected: sesMessageTagsHeaderValue('postSigninRecoveryPhone') }],
      ['X-Template-Name', { test: 'equal', expected: 'postSigninRecoveryPhone' }],
      ['X-Template-Version', { test: 'equal', expected: TEMPLATE_VERSIONS.postSigninRecoveryPhone }],
    ])],
    ['html', [
      { test: 'include', expected: 'Your recovery phone was used to sign in' },
  ...
  ['postSigninRecoveryCodeEmail', new Map<string, Test | any>([
    ['subject', { test: 'equal', expected: 'Backup authentication code used to sign in' }],
    ['headers', new Map([
      ['X-SES-MESSAGE-TAGS', { test: 'equal', expected: sesMessageTagsHeaderValue('postSigninRecoveryCode') }],
      ['X-Template-Name', { test: 'equal', expected: 'postSigninRecoveryCode' }],
      ['X-Template-Version', { test: 'equal', expected: TEMPLATE_VERSIONS.postSigninRecoveryCode }],
    ])],
    ['html', [
      { test: 'include', expected: 'Your backup authentication code was used to sign in' },
      { test: 'include', expected: 'If you didn’t do this, you should change your password immediately to keep your account safe.' },
      { test: 'include', expected: 'You signed in from:' },
      { test: 'include', expected: `${MESSAGE.device.uaBrowser} on ${MESSAGE.device.uaOS} ${MESSAGE.device.uaOSVersion}` },
      { test: 'include', expected: `${MESSAGE.location.city}, ${MESSAGE.location.stateCode}, ${MESSAGE.location.country} (estimated)` },
  ...
  ['subscriptionFirstInvoiceEmail', new Map<string, Test | any>([
    ...
    ['html', [
      ...
      { test: 'include', expected: `${MESSAGE_FORMATTED.invoiceAmountDue}` },
      { test: 'include', expected: `Discount` },
      { test: 'include', expected: `${MESSAGE_FORMATTED.invoiceDiscountAmount}` },
      { test: 'include', expected: `<b>Date:</b> March 20, 2020` },
      { test: 'include', expected: `Your next invoice will be issued on April 19, 2020` },
      { test: 'include', expected: `View invoice` },
      { test: 'notInclude', expected: 'utm_source=email' },
    ...
    ['text', [
      ...
      { test: 'include', expected: `Invoice number: ${MESSAGE.invoiceNumber}` },
      { test: 'include', expected: `Mastercard ending in 5309` },
      { test: 'include', expected: `Amount paid: ${MESSAGE_FORMATTED.invoiceAmountDue}` },
  ...
  ['subscriptionFirstInvoiceEmail', new Map<string, Test | any>([
    ...
    ['html', [
      ...
      { test: 'include', expected: `${MESSAGE_FORMATTED.invoiceTaxAmount}` },
      { test: 'include', expected: `Amount paid` },
      { test: 'include', expected: `${MESSAGE_FORMATTED.invoiceAmountDue}` },
  ...
  ['subscriptionFirstInvoiceEmail', new Map<string, Test | any>([
    ...
    {...x, remainingAmountTotalInCents: 500, showTaxAmount: true })}
  ],
    ...
    {...x, remainingAmountTotalInCents: 500, showTaxAmount: true, payment_provider: 'link' })}
  ],
  ['subscriptionFirstInvoiceEmail', new Map<string, Test | any>([
    ['subject', { test: 'equal', expected: `${MESSAGE.productName} payment confirmed` }],
    ['headers', new Map([
    ...
    {...x, remainingAmountTotalInCents: 500, showTaxAmount: true, payment_provider: 'apple_pay' })}
  ],
  ['subscriptionFirstInvoiceEmail', new Map<string, Test | any>([
    ['subject', { test: 'equal', expected: `${MESSAGE.productName} payment confirmed` }],
    ['headers', new Map([
    ...
    {...x, remainingAmountTotalInCents: 500, showTaxAmount: true, payment_provider: 'google_pay', paymentProviderName: 'Google Pay' })}
  ],
  ...
  ['subscriptionRenewalReminderEmail', new Map<string, Test | any>([
    ...
    ['html', [
      ...
      { test: 'include', expected: `At that time, Mozilla will renew your ${MESSAGE.planInterval} subscription and a charge of ${MESSAGE_FORMATTED.invoiceTotal} will be applied to the payment method on your account.` },
  ...
  ['subscriptionRenewalReminderEmail', new Map<string, Test | any>([
    ...
    ['html', [
      ...
      { test: 'include', expected: `Dear ${MESSAGE.subscription.productName} customer` },
      { test: 'include', expected: `Your current subscription is set to automatically renew in ${MESSAGE.reminderLength} days.` },
      { test: 'include', expected: `At that time, Mozilla will renew your ${MESSAGE.planInterval} subscription and a charge of ${MESSAGE_FORMATTED.invoiceSubtotal} + ${MESSAGE_FORMATTED.invoiceTaxAmount} tax will be applied to the payment method on your account.` },
      { test: 'notInclude', expected: `At that time, Mozilla will renew your ${MESSAGE.planInterval} subscription and a charge of ${MESSAGE_FORMATTED.invoiceTotal} will be applied to the payment method on your account.` },
      { test: 'include', expected: "Sincerely," },
      { test: 'include', expected: `The ${MESSAGE.subscription.productName} team` },
      { test: 'notInclude', expected: 'utm_source=email' },
    ]],
  ...
  ]), {updateTemplateValues: x => ({...x, productName: MESSAGE.subscription.productName, invoiceTaxInCents: MESSAGE.invoiceTaxAmountInCents, invoiceSubtotalInCents: MESSAGE.invoiceSubtotalInCents })}],
  ...
  ]), {updateTemplateValues: x => ({...x, productName: MESSAGE.subscription.productName, hadDiscount: true, hasDifferentDiscount: false, invoiceTaxInCents: MESSAGE.invoiceTaxAmountInCents, invoiceSubtotalInCents: MESSAGE.invoiceSubtotalInCents })}],
  ...
  ]), {updateTemplateValues: x => ({...x, productName: MESSAGE.subscription.productName, hadDiscount: false, hasDifferentDiscount: true, invoiceTaxInCents: MESSAGE.invoiceTaxAmountInCents, invoiceSubtotalInCents: MESSAGE.invoiceSubtotalInCents  })}],
  ...
  ['subscriptionSubsequentInvoiceEmail', new Map<string, Test | any>([
    ...
    ['html', [
      ...
      { test: 'include', expected: `<b>Invoice number:</b> ${MESSAGE.invoiceNumber}` },
      { test: 'include', expected: `Mastercard ending in 5309` },
      { test: 'include', expected: `You have received an account credit of ${MESSAGE_FORMATTED.invoiceTotal}, which will be applied to your future invoices.`},
      { test: 'notInclude', expected: `Charged ${MESSAGE_FORMATTED.invoiceAmountDue2} on 03/20/2020` },
      { test: 'include', expected: `Your next invoice will be issued on April 19, 2020` },
      { test: 'include', expected: `View invoice` },
      { test: 'notInclude', expected: 'utm_source=email' },
      { test: 'notInclude', expected: 'PayPal' },
  ...
  ['subscriptionSubsequentInvoiceEmail', new Map<string, Test | any>([
      ...
      ['html', [
        ...
        { test: 'include', expected: `<b>Invoice number:</b> ${MESSAGE.invoiceNumber}` },
        { test: 'include', expected: `Amount paid` },
        { test: 'include', expected: `${MESSAGE_FORMATTED.invoiceAmountDue3}` },
        { test: 'include', expected: `Discount` },
        { test: 'include', expected: `${MESSAGE_FORMATTED.invoiceDiscountAmount}` },
        { test: 'include', expected: `<b>Date:</b> March 20, 2020` },
        { test: 'include', expected: `Your next invoice will be issued on April 19, 2020` },
      ...
      ['text', [
        ...
        { test: 'include', expected: `latest payment for ${MESSAGE.productName}.` },
        { test: 'include', expected: `Invoice number: ${MESSAGE.invoiceNumber}` },
        { test: 'include', expected: `Amount paid: ${MESSAGE_FORMATTED.invoiceAmountDue3}` },
        { test: 'include', expected: `Discount: ${MESSAGE_FORMATTED.invoiceDiscountAmount}` },
        { test: 'include', expected: `Date: March 20, 2020` },
        { test: 'include', expected: `Your next invoice will be issued on April 19, 2020` },
        { test: 'include', expected: `View Invoice: ${MESSAGE.invoiceLink}` },
  ...
  ['subscriptionUpgradeEmail', new Map<string, Test | any>([
    ...
    ['html', [
      ...
      { test: 'include', expected: `The previous rate was ${MESSAGE_FORMATTED.paymentAmountOld} per ${MESSAGE.productPaymentCycleOld}.`},
      { test: 'include', expected: `Going forward, you will be charged ${MESSAGE_FORMATTED.paymentAmountNew} per ${MESSAGE.productPaymentCycleNew}` },
      { test: 'include', expected: `You have been charged a one-time fee of ${MESSAGE_FORMATTED.invoiceAmountDue2} to reflect your subscription’s higher price for the remainder of this billing period (${MESSAGE.productPaymentCycleOld}).` },
      ...
      { test: 'notInclude', expected: `You have received an account credit in the amount of ${MESSAGE_FORMATTED.paymentProrated}.` },
      { test: 'notInclude', expected: 'utm_source=email' },
    ]],
    ['text', [
      { test: 'include', expected: `You have upgraded to ${MESSAGE.productNameNew}` },
    ...
    {...x, productName: MESSAGE.productNameNew, paymentProratedInCents: 523100 })}
  ],
    ...
    ['html', [
      ...
      { test: 'include', expected: decodeUrl(configHref('subscriptionTermsUrl', 'subscription-upgrade', 'subscription-terms')) },
      { test: 'include', expected: `You have successfully upgraded to ${MESSAGE.productNameNew}.` },
      { test: 'include', expected: `You have received an account credit in the amount of ${MESSAGE_FORMATTED.paymentProrated}.` },
      { test: 'include', expected: `The previous rate was ${MESSAGE_FORMATTED.paymentAmountOld} per ${MESSAGE.productPaymentCycleOld}.`},
      { test: 'include', expected: `Going forward, you will be charged ${MESSAGE_FORMATTED.paymentAmountNew} per ${MESSAGE.productPaymentCycleNew}` },
      { test: 'notInclude', expected: `one-time fee of ${MESSAGE_FORMATTED.invoiceAmountDue} to reflect your subscription’s higher price for the remainder of this billing period (${MESSAGE.productPaymentCycleOld}).` },
      { test: 'notInclude', expected: `one-time fee of ${MESSAGE_FORMATTED.paymentProrated} to reflect the higher charge for the remainder of this ${MESSAGE.productPaymentCycleOld}.` },
      { test: 'notInclude', expected: 'utm_source=email' },
    ]],
    ['text', [
    ...
    {...x, productName: MESSAGE.productNameNew, paymentProratedInCents: 0 })}
  ],
