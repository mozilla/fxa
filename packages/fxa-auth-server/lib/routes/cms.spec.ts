...
describe('cms', () => {
  ...
  describe('GET /cms/default', () => {
    beforeEach(() => {
      ...
      mockDefaultCmsManager.fetchDefault.mockReset();
      mockStatsD.increment.mockClear();
    });

    it('returns the default payload when present', async () => {
      mockDefaultCmsManager.fetchDefault.mockResolvedValue({
        default: {
      ...
      expect(mockDefaultCmsManager.fetchDefault).toHaveBeenCalledTimes(1);
      expect(mockStatsD.increment).toHaveBeenCalledWith(
        'cms.getDefault.success'
      );
    });

    it('returns {} when no entry exists', async () => {
      mockDefaultCmsManager.fetchDefault.mockResolvedValue({ default: null });
      request = { log };

      const response = await route.handler(request);

      expect(response).toEqual({});
      expect(mockStatsD.increment).toHaveBeenCalledWith('cms.getDefault.empty');
    });

    it('returns {} and logs on manager error', async () => {
      mockDefaultCmsManager.fetchDefault.mockRejectedValue(new Error('boom'));
      request = { log };

      const response = await route.handler(request);

      expect(response).toEqual({});
      expect(mockStatsD.increment).toHaveBeenCalledWith('cms.getDefault.error');
    });
  });
