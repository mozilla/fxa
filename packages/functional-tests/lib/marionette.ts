...
export class MarionetteClient {
   ...
   */
  async connect(retries = 10, delay = 2000): Promise<Record<string, unknown>> {
  ...
  private async sendCommand(
    ...
    if (data[2]) {
      ...
      throw new MarionetteError(error.error ?? '?', error.message ?? '?');
    }
  ...
  async executeAsyncScript(
    ...
    const result = await this.sendCommand('WebDriver:ExecuteAsyncScript', {
      script,
      args: options.args ?? [],
      sandbox: options.sandbox ?? 'default',
      newSandbox: options.newSandbox ?? true,
  ...
  async getElementAttribute(
    ...
    const result = await this.sendCommand('WebDriver:GetElementAttribute', {
      id: elementId,
      name,
    });
