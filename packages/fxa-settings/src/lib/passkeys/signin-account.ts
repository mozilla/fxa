...
export async function resolveSignedInAccount({
...
}): Promise<{ email: string; accountHasTotp: boolean } | undefined> {
  ...
  // The auth-client return type isn't formally typed; this is the subset used.
  const email = account?.emails?.find(
    (e: { email: string; isPrimary: boolean }) => e.isPrimary
  )?.email;
