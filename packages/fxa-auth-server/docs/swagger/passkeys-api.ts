...
const PASSKEY_VERIFICATION_FINISH_POST = {
  ...
  notes: [
    dedent`
      ...
      **Response:** \`mfaToken\`, whose claims carry the scope the challenge
      was created with (\`mfa:<scope>\`), the caller's session (\`stid\`), and
      the credential that signed the assertion (\`cid\`).
