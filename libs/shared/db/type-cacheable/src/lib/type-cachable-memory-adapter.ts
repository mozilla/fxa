...
export class MemoryAdapter implements CacheClient {
  ...
  public async get(cacheKey: string): Promise<any> {
    const cacheEntry = this.memcache.get(cacheKey);
  ...
  public async delHash(_: string | string[]): Promise<any> {
    throw new Error('delHash() not supported for cachable MemoryAdapter');
  }
}
...
export const useMemoryAdapter = (
  ...
  if (asFallback) {
    cacheManager.setFallbackClient(memoryAdapter);
  } else {
    cacheManager.setClient(memoryAdapter);
  }
