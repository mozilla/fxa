...
next-payment-confirm-with-legal-links-static-3 = Ovlašćujem { -brand-mozilla } da tereti moj način plaćanja za prikazani iznos, u skladu s <termsOfServiceLink>uvjetima usluge</termsOfServiceLink> i <privacyNoticeLink>napomenama o privatnosti</privacyNoticeLink>, sve dok ne otkažem pretplatu.
...
next-charge-with-tax-card = Tvoj sljedeća naplata će iznositi { $nextInvoiceTotal } + { $taxDue } porez na karticu koja završava na { $last4 }, { $currentPeriodEnd }.
next-charge-with-tax-payment-method = Tvoj sljedeća naplata će iznositi { $nextInvoiceTotal } + { $taxDue } porez na tvoj način plaćanja putem usluge { $paymentMethod }, { $currentPeriodEnd }.
next-charge-with-tax = Tvoj sljedeća naplata će iznositi { $nextInvoiceTotal } + { $taxDue } porez { $currentPeriodEnd }.
next-charge-no-tax-card = Tvoj sljedeća naplata će iznositi { $nextInvoiceTotal } na karticu koja završava na { $last4 }, { $currentPeriodEnd }.
next-charge-no-tax-payment-method = Tvoj sljedeća naplata će iznositi { $nextInvoiceTotal } na tvoj način plaćanja putem usluge { $paymentMethod }, { $currentPeriodEnd }.
next-charge-no-tax = Tvoj sljedeća naplata će iznositi { $nextInvoiceTotal } { $currentPeriodEnd }.
