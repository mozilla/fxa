...
/** Caps the set and the batch loop, which the byte limit alone does not bound. */
const SYNC_MAX_ENTRIES = 250_000;
...
export class DomainBlocklistController {
  ...
  public async sync(
    ...
    // Dropped as invalid or repeated; log the count only, never the entries.
    const skipped = total - domains.length;
    ...
    if (skipped > 0) {
      this.log.warn('domainBlocklist.sync.skipped', { user, skipped });
    }
    return { ok: true, total, submitted: domains.length };
  }

  /** Accepts https only. A public list url needs no credentials, query, or fragment, and the audit log keeps whatever is posted. */
  ...
  private parseList(body: string): { total: number; domains: string[] } {
    ...
    for (const line of body.split(/\r?\n/)) {
      ...
      total++;
      if (total > SYNC_MAX_ENTRIES) {
        throw new BadRequestException(
          `List has more than ${SYNC_MAX_ENTRIES} entries`
        );
      }
