...
describe('/recovery_phone', () => {
  ...
  describe('POST /recovery_phone/signin/send_code', () => {
    it('sends recovery phone code', async () => {
      ...
      expect(mockRecoveryPhoneService.sendCode.mock.calls[0][0]).toBe(uid);
      ...
      expect(mockCustoms.checkAuthenticated.mock.calls[0][1]).toBe(uid);
      expect(mockCustoms.checkAuthenticated.mock.calls[0][2]).toBe(email);
      expect(mockCustoms.checkAuthenticated.mock.calls[0][3]).toBe(
  ...
  describe('POST /recovery_phone/reset_password/send_code', () => {
    it('sends recovery phone code', async () => {
      ...
      expect(
        mockGlean.resetPassword.recoveryPhoneCodeSendError.mock.calls.length
      ).toBe(0);
  ...
  describe('POST /recovery_phone/create', () => {
    it('creates recovery phone number', async () => {
      ...
      expect(mockRecoveryPhoneService.setupPhoneNumber.mock.calls[0][0]).toBe(
        uid
      );
      expect(mockRecoveryPhoneService.setupPhoneNumber.mock.calls[0][1]).toBe(
        phoneNumber
      );
      expect(mockRecoveryPhoneService.getNationalFormat).toHaveBeenCalledTimes(
        1
      );
      expect(mockRecoveryPhoneService.getNationalFormat.mock.calls[0][0]).toBe(
        phoneNumber
      );
      expect(mockGlean.twoStepAuthPhoneCode.sent).toHaveBeenCalledTimes(1);
      expect(mockGlean.twoStepAuthPhoneCode.sendError).toHaveBeenCalledTimes(0);
  ...
  describe('POST /recovery_phone/confirm', () => {
    it('confirms a code with TOTP enabled – sends post-add email', async () => {
      ...
      expect(mockRecoveryPhoneService.confirmSetupCode.mock.calls[0][0]).toBe(
        uid
      );
      expect(mockRecoveryPhoneService.confirmSetupCode.mock.calls[0][1]).toBe(
        code
      );
  ...
  describe('POST /recovery_phone/signin/confirm', () => {
    it('confirms a code during signin', async () => {
      ...
      expect(mockRecoveryPhoneService.confirmCode.mock.calls[0][0]).toBe(uid);
      expect(mockRecoveryPhoneService.confirmCode.mock.calls[0][1]).toBe(code);
  ...
  describe('POST /recovery_phone/reset_password/confirm', () => {
    it('successfully confirms the code', async () => {
      ...
      expect(mockRecoveryPhoneService.confirmCode.mock.calls[0][1]).toBe(code);
      expect(
        mockGlean.resetPassword.recoveryPhoneCodeComplete.mock.calls.length
      ).toBe(1);
      expect(
        mockAccountEventsManager.recordSecurityEvent
      ).toHaveBeenCalledTimes(1);
  ...
  describe('DELETE /recovery_phone', () => {
    it('removes a recovery phone', async () => {
      ...
      expect(mockRecoveryPhoneService.removePhoneNumber.mock.calls[0][0]).toBe(
        uid
      );
  ...
  describe('GET /recovery_phone', () => {
    it('gets a recovery phone', async () => {
      ...
      expect(mockRecoveryPhoneService.hasConfirmed.mock.calls[0][0]).toBe(uid);
    });
    ...
    it('returns masked phone number for unverified session', async () => {
      ...
      expect(mockRecoveryPhoneService.hasConfirmed).toHaveBeenCalledTimes(1);
      expect(mockRecoveryPhoneService.hasConfirmed.mock.calls[0][0]).toBe(uid);
      expect(mockRecoveryPhoneService.hasConfirmed.mock.calls[0][1]).toBe(4);
    });
  });
  ...
  describe('POST /recovery_phone/message_status', () => {
    it('handles a message status update from twilio using X-Twilio-Signature header', async () => {
      ...
      expect(
        mockRecoveryPhoneService.validateTwilioWebhookCallback.mock.calls.length
      ).toBe(1);
      expect(
        mockRecoveryPhoneService.validateTwilioWebhookCallback.mock.calls[0][0]
      ).toEqual({
        twilio: {
      ...
      expect(
        mockRecoveryPhoneService.onMessageStatusUpdate.mock.calls[0][0]
      ).toBe(payload);
    });
