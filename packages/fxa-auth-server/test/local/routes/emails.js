...
describe('/recovery_email/resend_code', () => {
  ...
  it('verification', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      ...
      assert.equal(fxaMailer.sendVerifyEmail.callCount, 1);
      const args = fxaMailer.sendVerifyEmail.args[0];
      ...
      fxaMailer.sendVerifyEmail.resetHistory();
  ...
  it('confirmation', () => {
    ...
    return runTest(route, mockRequest, (response) => {
      ...
      assert.equal(fxaMailer.sendVerifyLoginEmail.callCount, 1);
      const args = fxaMailer.sendVerifyLoginEmail.args[0];
      assert.equal(args[0].device.uaBrowser, 'Firefox');
...
describe('/mfa/recovery_email/secondary/resend_code', () => {
  ...
  it('errors when reservation belongs to a different uid', async () => {
    ...
    const routes = makeRoutes({ authServerCacheRedis, mailer: mockMailer, db: mockDB }, {});
    const route = getRoute(routes, '/mfa/recovery_email/secondary/resend_code');
    const request = mocks.mockRequest({
      credentials: { uid, email: TEST_EMAIL },
      payload: { email },
