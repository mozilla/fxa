...
export const InlineRecoverySetupContainer = ({
  ...
  useEffect(() => {
    async function acctRefresh() {
      // Always refresh to ensure we have fresh recoveryPhone.available from the server.
      // The recoveryPhone.available field determines if phone choice is shown,
      // and it's not populated when account data is first created during sign-in.
  ...
  // TOTP status state (migrated from GraphQL)
