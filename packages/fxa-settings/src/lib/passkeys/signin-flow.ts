...
import { clearPendingPasskeyWrap } from './pending-wrap';
...
export function usePasskeySignIn({
  ...
  const onClick = useCallback(async () => {
    ...
    // Desktop browser sign-ins that need encryption keys and so end in a
    // password step can offer to store a passkey wrap afterwards: that is the
    // only flow where `kB` is derived client-side. Mobile clients close the
    // web view at handoff, so the page that makes the offer would never be
    // seen there.
    const offerPasswordFreeSetup =
    ...
    try {
      ...
      const challengeOptions = await authClient.beginPasskeyAuthentication({
        keysRequired,
        ...(offerPasswordFreeSetup ? { scope: 'passkey' } : {}),
      });
      ...
      const prfOut = offerPasswordFreeSetup
