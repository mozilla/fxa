...
describe('StripeAutomaticTaxConverterHelpers', () => {
  ...
  describe('isLocalIP', () => {
    it('returns true for class A', () => {
      expect(helpers.isLocalIP('10.0.0.1')).toBe(true);
    });

    it('returns true for class B', () => {
      expect(helpers.isLocalIP('172.16.0.1')).toBe(true);
    });

    it('returns true for class C', () => {
      expect(helpers.isLocalIP('192.168.0.1')).toBe(true);
    });

    it('returns false for non-local IP', () => {
      expect(helpers.isLocalIP('1.1.1.1')).toBe(false);
    });
  });
  ...
  describe('filterEligibleSubscriptions', () => {
    ...
    it('filters via helper methods', () => {
      expect(willBeRenewed.mock.calls[0][0]).toEqual(subscription1);
      expect(isStripeTaxDisabled.mock.calls[0][0]).toEqual(subscription1);
      expect(isWithinNoticePeriod.mock.calls[0][0]).toEqual(subscription1);
    });
