...
button-passkey-signin = Z gronidłowym klucom pśizjawiś
button-passkey-signin-loading = Pśizjawja se wěsće…
...
dark-mode-toggle-label = Drastwu změniś
...
passkey-sub-row-created-date = Napórany: { $createdDate }
...
terms-privacy-agreement-intro-3 = Gaž pókšacujośo, zwólijośo do slědujego:
terms-privacy-agreement-customized-terms = { $serviceName }: <termsLink>Wužywańske wuměnjenja</termsLink> a <privacyLink>powěźeńka priwatnosći</privacyLink>
terms-privacy-agreement-mozilla-2 = { -product-mozilla-accounts(capitalization: "uppercase") } <mozillaAccountsTos>Wužywańske wuměnjenja</mozillaAccountsTos> a <mozillaAccountsPrivacy>powěźeńki priwatnosći</mozillaAccountsPrivacy>
...
pair-auth-allow-refuse-device-link = Jolic njejsćo to był wy, <a>změńśo swójo gronidło</a>
...
signin-passwordless-code-heading = Wobkšuśeński kod zapódaś
signin-passwordless-code-subheading = Pśizjawjenje jano jaden kšac trajo, gaž toś ten kod wužywaśo.
signin-passwordless-code-instruction =
    { $expirationMinutes ->
        [one] Zapódajśo kod, kótaryž jo se pósłał za { $expirationMinutes } minutu na <email>{ $email }</email>.
        [two] Zapódajśo kod, kótaryž jo se pósłał za { $expirationMinutes } minuśe na <email>{ $email }</email>.
...
signin-passwordless-code-resend-link = Nowy kod pśez e-mail pósłaś.
signin-passwordless-code-resend-error = Něco njejo se raźiło. Nowy code njedajo se słaś.
signin-passwordless-code-other-account-link = Wužywajśo druge konto
