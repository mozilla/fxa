...
describe('MeteringWebhookManager', () => {
  ...
  function args(overrides: Partial<WebhookDispatchArgs> = {}): WebhookDispatchArgs {
    return {
  ...
  describe('dispatch', () => {
    it('produces a different signature header when the client secret is rotated', async () => {
      const first = await captureSignatureHeader(
        meteringWebhookManager
      );
      const rotated = await build({
        openmeterBaseUrl: 'http://example.com',
        clients: { [SIGNING_CLIENT_ID]: 'different-secret-with-enough-entropy-bb' },
      });
      fetchMock.mockClear();
    ...
    it('POSTs to the configured URL with the signed envelope and idempotency key', async () => {
      ...
      expect(init.headers['X-Entitlements-Metering-Signature']).toMatch(/^v1=[0-9a-f]+$/);
      ...
      expect(body.previousUsage).toBeUndefined();
    ...
    it('throws on 5xx so Cloud Tasks can re-enqueue', async () => {
      ...
      await expect(
        meteringWebhookManager.dispatch(args())
      ).rejects.toThrow(/Webhook target returned 503/);
      expect(fetchMock).toHaveBeenCalledTimes(1);
    });
    ...
    it('throws on 4xx so Cloud Tasks can re-enqueue (and eventually DLQ)', async () => {
      ...
      await expect(
        meteringWebhookManager.dispatch(args())
      ).rejects.toThrow(/Webhook target returned 400/);
      expect(fetchMock).toHaveBeenCalledTimes(1);
    });
    ...
    it('skips dispatch silently when there is no signing key for the signingClientId', async () => {
      await expect(
        meteringWebhookManager.dispatch(args({ signingClientId: 'no-such-client' }))
      ).resolves.toBeUndefined();
