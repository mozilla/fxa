...
const Supp = ({
  ...
  // Fixed on the first settled fxa_status. useFxAStatus lets a late reply
  // overturn its 'unanswered' default, and switching to v2 after the v1 channel
  // has opened would leave the v2 screens driving a v1 state machine.
  const protocolRef = useRef<1 | 2 | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (protocolRef.current === undefined) {
      protocolRef.current =
        authorityWantsV2 && supplicantPairingVersion === 2 ? 2 : 1;
    }
