...
import { Email, LinkedAccount, SecurityEvent, RawAttachedClient, mapAttachedClient } from '../../models/Account';
  ...
  const fetchAccountData = useCallback(async () => {
    ...
    try {
      ...
      if (attachedClientsResult.status === 'fulfilled') {
        accountData.attachedClients = (attachedClientsResult.value as RawAttachedClient[]).map(mapAttachedClient);
      } else {
      ...
      try {
        ...
        switch (field) {
          case 'attachedClients': {
            ...
            fieldData.attachedClients = (clients as RawAttachedClient[]).map(mapAttachedClient);
            break;
          }
          case 'displayName':
          case 'avatar': {
