...
- **Option I: Use the session token with full scopes via `fxa-credentials` grant**: Reuse the existing custom grant type, passing a session token and the complete desired scope set
...
- **Scoped keys problem**: The `fxa-credentials` grant runs through `validateRequestedGrant`, which checks `hasScopedKeys` and requires a verified token for key-bearing scopes like `oldsync`. Passing the full scope set including `oldsync` hits this code path. The token exchange flow bypasses `validateRequestedGrant` entirely since the existing refresh token already proves the user authorized those scopes.
...
- **Scoped keys ambiguity**: If the full scope set includes `oldsync`, the server would need to distinguish "this scope was already granted" from "the client is requesting new key-bearing access," adding unnecessary complexity to the token exchange path.

Both Options I and II share a common disadvantage versus Option III:

- **Custom grant type is needed regardless**: RFC 6749 Section 6 prohibits scope expansion on standard `grant_type=refresh_token` requests, and `node-oidc-provider` enforces this. The `fxa-credentials` grant already requires custom registration via `registerGrantType()`. RFC 8693's `urn:ietf:params:oauth:grant-type:token-exchange` is a recognized grant type with documented recipes in `node-oidc-provider`, making it a clean fit.
