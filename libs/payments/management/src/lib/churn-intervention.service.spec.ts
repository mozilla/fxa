...
import { ChurnInterventionRecordManager, ChurnInterventionEntryFactory } from '@fxa/payments/cart';
...
describe('ChurnInterventionService', () => {
  ...
  let churnInterventionRecordManager: ChurnInterventionRecordManager;
  ...
  beforeEach(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: ChurnInterventionRecordManager,
          useValue: { getEntry: jest.fn() }
        },
        {
    ...
    churnInterventionRecordManager = moduleRef.get(ChurnInterventionRecordManager);
  ...
  describe('getEntry', () => {
    it('returns the churn intervention entry', async () => {
      (churnInterventionRecordManager.getEntry as jest.Mock).mockResolvedValue(mockEntry);
      const result = await churnInterventionService.getEntry(mockEntry.customerId, mockEntry.churnInterventionId);

      expect(churnInterventionRecordManager.getEntry).toHaveBeenCalledWith(
  ...
  describe('determineStaySubscribedEligibility', () => {
    ...
    it('returns ineligible when redemption limit reached', async () => {
      ...
      (churnInterventionRecordManager.getEntry as jest.Mock)
        .mockResolvedValue({ redemptionCount: 1 });
    ...
    it('returns ineligible when subscription is not active', async () => {
      ...
      (productConfigurationManager.getChurnInterventionBySubscription as jest.Mock)
        .mockResolvedValue(cmsResult([entry]));
      (churnInterventionRecordManager.getEntry as jest.Mock)
        .mockResolvedValue({ redemptionCount: 0 });
      (subscriptionManagementService.getSubscriptionStatus as jest.Mock)
        .mockResolvedValue({ active: false, cancel_at_period_end: true });
    ...
    it('returns ineligible when subscription is still active', async () => {
      ...
      (churnInterventionRecordManager.getEntry as jest.Mock)
        .mockResolvedValue({ redemptionCount: 0 });
      (subscriptionManagementService.getSubscriptionStatus as jest.Mock)
        .mockResolvedValue({ active: true, cancel_at_period_end: false });
    ...
    it('returns eligible on success', async () => {
      ...
      (churnInterventionRecordManager.getEntry as jest.Mock)
        .mockResolvedValue({ redemptionCount: 0 });
      (subscriptionManagementService.getSubscriptionStatus as jest.Mock)
        .mockResolvedValue({ active: true, cancel_at_period_end: true });
