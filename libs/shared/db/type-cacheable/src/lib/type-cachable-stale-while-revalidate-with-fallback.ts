...
export class StaleWhileRevalidateWithFallbackStrategy implements CacheStrategy {
  ...
  async handle(context: CacheStrategyContext): Promise<unknown> {
    ...
    const staleBefore = Date.now() - this.staleValueTtl * 1000;
    const result = await Promise.any([cachedValuePromise, methodPromise]).catch(
      (e) => {
        // Do nothing, we handle this below
      }
    );
    if (cachedValueDone && result && result.timestamp > staleBefore) {
