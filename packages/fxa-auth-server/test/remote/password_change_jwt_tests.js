...
describe('#integration - remote password change JWT', function () {
  ...
  [{ version: '' }, { version: 'V2' }].forEach((testOptions) => {
    describe(`#integration${testOptions.version} - remote password change with JWT`, function () {
      ...
      it('should reject JWT with wrong session token ownership', async function() {
        ...
        // Get session token from client2 for JWT generation
        const sessionTokenHex2 = client2.sessionToken;
        const sessionToken2 = require('../../lib/tokens')({
          trace: function() {}
        }).SessionToken.fromHex(sessionTokenHex2);
        const sessionTokenId2 = (await sessionToken2).id;

        // Generate JWT with client1's UID but client2's session token ID
        const now = Math.floor(Date.now() / 1000);
        const claims = {
          sub: client1.uid, // Client1's UID
          scope: ['mfa:password'],
          iat: now,
          jti: uuid.v4(),
          stid: sessionTokenId2, // But client2's session token!
        };
        ...
        // Prepare new password credentials for client1
        const newCreds = await client1.setupCredentials(email1, newPassword);
        client1.deriveWrapKbFromKb();

        const payload = {
          authPW: newCreds.authPW.toString('hex'),
          wrapKb: client1.wrapKb,
          clientSalt: client1.clientSalt,
        };

        // Should reject due to session ownership mismatch
        try {
          await client1.changePasswordJWT(jwtToken, payload);
          assert.fail('Should have rejected JWT with wrong session ownership');
        } catch (err) {
          // This should fail because the session doesn't belong to the user in the JWT sub claim
          assert.equal(err.errno, 110, "Unauthorized"); // Invalid token
        }
      });
    })
  })
