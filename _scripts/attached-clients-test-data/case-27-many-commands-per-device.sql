-- Test Case 27: Many deviceCommands per device
-- UID: 11111111111111111111111111111127
-- Description: 100 devices, each with 20+ commands
...
SET @uid_hex = '11111111111111111111111111111127';
SET @uid = UNHEX(@uid_hex);

-- Create OAuth client for refresh tokens
SET @client_id = UNHEX(SUBSTRING(MD5(CONCAT('client_case27', @uid_hex)), 1, 16));
INSERT IGNORE INTO fxa_oauth.clients (id, name, hashedSecret, redirectUri, imageUri, canGrant, publicClient, trusted)
VALUES (@client_id, 'Test Client Case 27', UNHEX(MD5('secret')), 'https://example.com/callback', 'https://example.com/icon.png', 0, 0, 0);

-- Ensure all command identifiers exist
INSERT IGNORE INTO deviceCommandIdentifiers (commandName) VALUES
  ...
  ('https://identity.mozilla.com/cmd/command16'),
  ('https://identity.mozilla.com/cmd/command17'),
  ('https://identity.mozilla.com/cmd/command18'),
  ('https://identity.mozilla.com/cmd/command19'),
  ('https://identity.mozilla.com/cmd/command20');

-- Insert 100 devices, each with 20 commands (idempotent - skips duplicates)
DELIMITER //
DROP PROCEDURE IF EXISTS insertCase27Devices //
CREATE PROCEDURE insertCase27Devices()
BEGIN
  ...
  WHILE device_idx < 100 DO
    SET device_id = UNHEX(SUBSTRING(MD5(CONCAT('device_case27_', device_idx, @uid_hex)), 1, 32));
    SET token_id = UNHEX(MD5(CONCAT('session_case27_', device_idx, @uid_hex)));
    SET token_data = UNHEX(MD5(CONCAT('data_case27_', device_idx, @uid_hex)));
    SET refresh_token = UNHEX(MD5(CONCAT('refresh_case27_', device_idx, @uid_hex)));
...
CALL insertCase27Devices();
DROP PROCEDURE insertCase27Devices;

SELECT CONCAT('Case 27 complete. UID: ', @uid_hex) AS status;
