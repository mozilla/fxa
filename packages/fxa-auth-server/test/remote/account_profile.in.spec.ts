...
describe.each(testVersions)(
  ...
  ({ version, tag }) => {
    ...
    // OAuth Bearer token tests are skipped: the child-process TestServer's
    // hawk-fxa-token auth scheme throws a non-Boom AppError for Bearer tokens,
    // which prevents Hapi from falling through to the oauthToken strategy.
    // The original Mocha tests ran in-process where this worked differently.
    describe.skip('when a request is authenticated with a valid oauth token', () => {
      let client: any;
      let token: string;
    ...
    describe('when the profile data is not default', () => {
      ...
      // Skipped: uses OAuth Bearer token (see skip comment above)
      describe.skip('when the account has TOTP', () => {
      ...
      // Skipped: uses OAuth Bearer token (see skip comment above)
      describe.skip('when the locale is empty', () => {
