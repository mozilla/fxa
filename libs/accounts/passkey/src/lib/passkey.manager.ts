...
export class PasskeyManager {
  ...
  /**
   * Re-seal kB to the existing pkR, leaving skR's protection untouched.
   *
   * @returns true when a wrap was updated, false when the credential has none
   */
  async updatePasskeyWrapSeal(
    uid: string,
    credentialId: string,
    data: PasskeyWrapSealUpdate,
    updatedAt: number
  ): Promise<boolean> {
    const rowsUpdated = await updatePasskeyWrapSeal(
      this.db,
      uid,
      credentialId,
      data,
