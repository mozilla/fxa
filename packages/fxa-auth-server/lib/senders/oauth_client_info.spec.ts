...
describe('lib/senders/oauth_client_info:', () => {
  describe('fetch:', () => {
    ...
    it('falls back to Mozilla if error', async () => {
      ...
      expect(res.name).toBe('Mozilla');
      expect(mockLog.fatal).toHaveBeenCalledTimes(1);
    });
    ...
    it('fetches and memory caches client information', async () => {
      ...
      expect(mockLog.trace).toHaveBeenNthCalledWith(1, 'fetch.start');
      expect(mockLog.trace).toHaveBeenNthCalledWith(
        2,
        'fetch.usedServer',
        expect.anything()
      );
      expect(mockLog.trace).toHaveBeenCalledTimes(2);

      // second call is cached
      const res2 = await fetch('24bdbfa45cd300c5');
      expect(mockLog.trace).toHaveBeenNthCalledWith(3, 'fetch.start');
      expect(mockLog.trace).toHaveBeenNthCalledWith(4, 'fetch.usedCache');
