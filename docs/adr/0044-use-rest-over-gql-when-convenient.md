# Use REST (auth-client) over GQL when

- Status: proposed
- Deciders: Lauren Zugai, Ben Bangert, Dan Schomburg, [you?]
- Date: 2024-12-11
...
This brings us to our question: what do we want to do in the meantime about the state of GQL and auth-client in FxA's front-end?
...
Chosen option: B, "Use auth-client for all new network requests, or prefer auth-client and only use GQL where already set up" because by standardizing on auth-client for now or at least using it when it is convenient, we minimize overhead in setting up and maintaining GQL mutations/resolvers while leaving open the possibility of moving to a framework like NextJS later, where we can reevaluate our data-fetching and state strategies more holistically.

This options allow engineers the most flexibility to get tasks out the quickest way, especially if we can create a DRY wrapper for auth-client like what's described in [FXA-10861](https://mozilla-hub.atlassian.net/browse/FXA-10861).
...
- Good, because we'll finally be able to realize the full benefits of using GQL
...
- Good, because we will avoid laborious back-end and front-end refactoring tasks now in favor of other work and revisiting a plan for consistency when we move to NextJS
