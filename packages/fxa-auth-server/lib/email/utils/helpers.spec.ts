...
describe('email utils helpers', () => {
  ...
  describe('logEmailEventSent', () => {
    it('should check headers case-insensitively', () => {
      ...
      expect(log.info.mock.calls[0][1].locale).toBe('ru');
    });
    ...
    it('should log an event per CC email', () => {
      ...
      expect(log.info.mock.calls[1][1].domain).toBe('gmail.com');
      expect(log.info.mock.calls[2][1].domain).toBe('yahoo.com');
    });
  });
  ...
  describe('logErrorIfHeadersAreWeirdOrMissing', () => {
    ...
    it('logs an error if message.mail is missing', () => {
      ...
      expect(log.error.mock.calls[0][0]).toBe('emailHeaders.missing');
    ...
    it('logs an error if message.mail.headers is non-object', () => {
      ...
      expect(log.error).toHaveBeenCalledTimes(1);
      expect(log.error.mock.calls[0][0]).toBe('emailHeaders.weird');
      expect(log.error.mock.calls[0][1]).toEqual({
        type: 'string',
        origin: 'wibble',
      });
