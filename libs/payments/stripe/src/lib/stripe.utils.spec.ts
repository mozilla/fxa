...
import { getSubscribedPlans, getSubscribedProductIds } from './stripe.utils';
...
describe('util', () => {
  describe('getSubscribedPlans', () => {
    it('returns plans successfully', async () => {
      ...
      const result = await getSubscribedPlans(mockSubscriptionList);
  ...
  describe('getSubscribedProductIds', () => {
    it('returns product IDs successfully', async () => {
      ...
      const result = await getSubscribedProductIds([mockPlan]);
      expect(result).toEqual([mockProductId]);
    });

    it('returns empty array if no subscriptions exist', async () => {
      const result = await getSubscribedProductIds([]);
      expect(result).toEqual([]);
    });
  });
});
