...
const meterKeyRowSchema = z.object({
  client_id: z.string(),
  slug: z.string(),
});
...
const FIND_ACTIVE_METERS_SQL = `
SELECT DISTINCT client_id, slug
FROM
(
    SELECT client_id, slug
    FROM ${EVENTS_TABLE}
    WHERE ingested_at >= {ingestedSince:DateTime64(3, 'UTC')}
    UNION ALL
    SELECT client_id, slug
...
SELECT
    -- Slices written in one run share updated_at. The merge keeps the last one
    -- inserted, so slices must be written ascending for this tie-break to agree
    formatDateTime(
        argMax(watermark, (updated_at, watermark)),
        '%Y-%m-%dT%H:%i:%S.%fZ',
        'UTC'
    ) AS watermark
FROM ${WATERMARKS_TABLE}
...
(
    ...
    WHERE client_id = {clientId:String}
      ...
      AND ingested_at < {ingestedUntil:DateTime64(3, 'UTC')}
...
SELECT
    ...
    formatDateTime(min(e.event_time), '%Y-%m-%dT%H:%i:%S.%fZ', 'UTC') AS started_at
FROM ${EVENTS_TABLE} AS e
LEFT JOIN last_session AS l ON e.subject = l.subject
WHERE e.client_id = {clientId:String}
  AND e.slug = {slug:String}
  AND e.ingested_at >= {ingestedSince:DateTime64(3, 'UTC')}
  AND e.ingested_at < {ingestedUntil:DateTime64(3, 'UTC')}
  AND e.event_time >= {eventTimeFloor:DateTime64(3, 'UTC')}
  AND e.event_time < {to:DateTime64(3, 'UTC')}
  -- Filter per event, not on min(event_time), or an in-session event hides a later post-expiry one
...
export class MeteringSweepRepository {
  ...
  async findActiveMeters(params: FindActiveMetersParams): Promise<MeterKey[]> {
    const rows = await this.clickHouseClient.query({
      sql: FIND_ACTIVE_METERS_SQL,
      rowSchema: meterKeyRowSchema,
      params: { ingestedSince: toClickHouseDateTime(params.ingestedSince) },
    });
    return rows.map((row) => ({ clientId: row.client_id, slug: row.slug }));
  }
  ...
  async findWindowCandidates(
    ...
    return this.clickHouseClient.query({
      ...
      params: {
        ...
        ingestedUntil: toClickHouseDateTime(params.ingestedUntil),
